...
- **Explicit native Typst backend selection (#200):** PDF builds continue to
  ...
  at build time with the `typst-inprocess` Cargo feature and at runtime with
  `--backend in-process`; feature-disabled selection fails without subprocess
  fallback. The adapter uses the existing `VirtualProject` resource boundary.
  Generated Typst diagnostics use validated lowerer source-map entries for
  original spans when available and never fabricate provenance.
  Cross-platform parity corpus expansion remains in #201; the temporary
  citationberg dependency pin remains in #203.
...
- **Bounded `.captionposition` document layout state (M3):** `.captionposition`
  ...
  contract, including preservation of nested caption-state mutations from
  successful argument evaluation. Positional, named, mixed, nullable `.none`,
  callable sharing, source-defined shadowing, source-backed binding failures,
  body rejection, atomic candidate evaluation, and old-IR serde defaults are
  covered. Upstream's block-body-to-final-`codeBlocks` fallback remains an
  explicit compatibility gap because the current frontend/IR boundary does
  not retain the required raw `DynamicValue` body text. Actual caption
  rendering and Typst/HTML placement remain deferred.
...
- **Bounded `.doclang` document state (M3):** `.doclang` now models upstream's
  ...
  getter behavior, callable sharing, source-defined shadowing, body rejection,
  source-backed failures, atomic replacement, Chinese provider routing, and
  Norwegian legacy `no_NO_NY` display identity are covered. Resolution is pure
  Rust with no host/JVM locale lookup; localization, hyphenation,
  locale-aware rendering, and the remaining document metadata family remain
  deferred.
...
- **Bounded author document state (M3):** `.docauthor` and `.docauthors` now
  share an evaluator-owned ordered author state. `.docauthors` accepts the
  bounded nested dictionary/list form, appends authors, preserves ordered
