...
def load_policy(path: Path) -> dict:
    ...
    for producer in producers:
        ...
        if declared_trigger == "pull_request_target":
            raise PolicyError(
                f"{path}: required producers must use unprivileged pull_request"
            )
    return policy
    ...
    for key, producer in producers.items():
        ...
        if producer["classification"] == "required":
            ...
            if trigger.types_restricted:
                ...
                if trigger.types is None or len(trigger.types) != len(canonical_types) or set(trigger.types) != canonical_types:
                    raise PolicyError(
