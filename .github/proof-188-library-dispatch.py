...
replace(
    "crates/arkst-project/src/source_store.rs",
...
replace(
    "crates/arkst-project/src/virtual_project.rs",
...
replace(
    ...
    "            }\n"
    "            let source_id = source_store.allocate_detached_id().map_err(|error| match error {\n"
...
replace(
    ...
    "    ) -> Self {\n"
    "        let mut context = Self::with_resources(\n"
    "            resources,\n"
    "            source_id,\n"
    "            source_mode,\n"
    "            metadata_defaults,\n"
...
old_method = """    pub fn evaluate_with_resources_for_mode<R: ResourceProvider>(
        &self,
        resources: &R,
        source_id: SourceId,
        source_mode: Mode,
...
"""
replace("crates/arkst-engine/src/evaluator.rs", old_method, new_method)
replace(
    "crates/arkst-engine/src/evaluator.rs",
...
Path("crates/arkst-project/tests/loadable_library_registry.rs").write_text(
    r'''use arkst_project::{BuildError, VirtualPathBuf, VirtualProjectBuilder};
...
fn registry_is_case_sensitive_and_does_not_change_project_source_ids() {
    ...
    for path in ["docs/main.qd", "docs/nested.qd"] {
        ...
        assert_eq!(baseline.sources().get_id(&path), project.sources().get_id(&path));
    }
    ...
    assert_eq!(project.loadable_library_name_by_source_id(alpha.source_id()), Some("alpha"));
...
fn registry_rejects_empty_and_duplicate_names_atomically() {
    let empty = VirtualProjectBuilder::new()
        ...
        .add_source("main.qd", "main").unwrap()
    ...
    let duplicate = VirtualProjectBuilder::new()
        ...
        .build();
    assert!(matches!(duplicate, Err(BuildError::DuplicateLoadableLibrary(name)) if name == "hello"));
}
'''
)

Path("crates/arkst-engine/tests/loadable_library_dispatch.rs").write_text(
    r'''use arkst_engine::{
...
impl ResourceProvider for FakeEnvironment {
    fn source_path(&self, source_id: SourceId) -> Option<String> {
        self.paths.get(&source_id).cloned().or_else(|| {
            self.libraries.values().find(|library| library.source_id == source_id)
                .map(|library| format!("@library/{}", library.name))
        })
    }
    fn read_text(&self, source_id: SourceId, reference: &str) -> Result<ResourceText, ResourceAccessError> {
        self.text_requests.borrow_mut().push((source_id, reference.to_string()));
        self.text.get(&(source_id, reference.to_string())).cloned()
            .unwrap_or_else(|| Err(ResourceAccessError::NotFound { path: reference.to_string() }))
    }
    fn read_source(&self, source_id: SourceId, reference: &str) -> Result<IncludedSource, ResourceAccessError> {
        self.source_requests.borrow_mut().push((source_id, reference.to_string()));
        self.sources.get(&(source_id, reference.to_string())).cloned()
            .ok_or_else(|| ResourceAccessError::NotFound { path: reference.to_string() })
    }
}
...
fn document(source: &str, source_id: SourceId) -> IrDocument {
    ...
    let (document, diagnostics) = ast_to_ir_with_diagnostics_for_mode(
        &parsed.document, source_id, &DocumentMetadataDefaults::default(), Mode::Quarkdown,
    );
    assert!(diagnostics.is_empty(), "{diagnostics:?}");
    document
}
fn evaluate(source: &str, source_id: SourceId, env: &FakeEnvironment) -> (IrDocument, Vec<arkst_diagnostics::Diagnostic>) {
    arkst_engine::evaluator::Evaluator::new().evaluate_with_resources_and_libraries_for_mode(
        env, env, source_id, Mode::Quarkdown, &document(source, source_id), &DocumentMetadataDefaults::default(),
    )
}
fn paragraph_text(document: &IrDocument) -> String {
    document.nodes.iter().filter_map(|node| match node {
        IrNode::Paragraph { content, .. } => Some(content.iter().filter_map(|inline| match inline {
            IrInline::Text { content, .. } => Some(content.as_str()), _ => None,
        }).collect::<String>()),
        _ => None,
    }).collect::<Vec<_>>().join("\n")
}
...
fn exact_library_name_precedes_file_lookup_and_ignores_sandbox() {
    ...
    env.sources.insert((main, "hello".into()), IncludedSource { path: "hello".into(), source_id: fallback, text: "wrong file".into() });
    env.sources.insert((main, "Hello".into()), IncludedSource { path: "Hello".into(), source_id: fallback, text: "file fallback".into() });
    env.libraries.insert("hello".into(), LoadableLibrarySource {
        name: "hello".into(), source_id: library, text: ".var {libvalue} {library}".into(),
    });
    let (result, diagnostics) = evaluate(".include {hello} sandbox:{subdocument}\n.libvalue", main, &env);
    assert!(diagnostics.is_empty(), "{diagnostics:?}");
    ...
    assert!(diagnostics.is_empty(), "{diagnostics:?}");
    assert_eq!(env.library_requests.borrow().as_slice(), &["Hello"]);
    assert_eq!(env.source_requests.borrow().as_slice(), &[(main, "Hello".into())]);
...
fn library_resource_access_keeps_includer_base_and_library_span_provenance() {
    ...
    env.text.insert((main, "data.txt".into()), Ok(ResourceText { path: "docs/data.txt".into(), text: "caller data".into() }));
    env.libraries.insert("reader".into(), LoadableLibrarySource { name: "reader".into(), source_id: reader, text: ".read {data.txt}".into() });
    env.libraries.insert("broken".into(), LoadableLibrarySource { name: "broken".into(), source_id: broken, text: ".read {missing.txt}".into() });
    let (result, diagnostics) = evaluate(".include {reader} sandbox:{scope}", main, &env);
    assert!(diagnostics.is_empty(), "{diagnostics:?}");
    assert_eq!(env.text_requests.borrow().as_slice(), &[(main, "data.txt".into())]);
    ...
    assert_eq!(env.text_requests.borrow().as_slice(), &[(main, "missing.txt".into())]);
...
fn includeall_uses_library_dispatch_per_element_then_file_fallback() {
    ...
    env.sources.insert((main, "local.qd".into()), IncludedSource { path: "local.qd".into(), source_id: local, text: ".libvalue".into() });
    env.libraries.insert("hello".into(), LoadableLibrarySource { name: "hello".into(), source_id: library, text: ".var {libvalue} {library}".into() });
    let (result, diagnostics) = evaluate(".includeall\n    - hello\n    - local.qd\n.libvalue", main, &env);
    assert!(diagnostics.is_empty(), "{diagnostics:?}");
    assert_eq!(env.library_requests.borrow().as_slice(), &["hello", "local.qd"]);
    assert_eq!(env.source_requests.borrow().as_slice(), &[(main, "local.qd".into())]);
    assert!(paragraph_text(&result).contains("library"));
}
'''
)

Path("crates/arkst-core/tests/loadable_library_compile.rs").write_text(
    r'''use arkst_core::ir::{IrInline, IrNode};
...
fn compile_wires_virtual_project_library_registry_before_local_file_lookup() {
    let project = VirtualProjectBuilder::new()
        ...
        .add_source("hello", "wrong file").unwrap()
...
fn compile_library_reads_from_the_includer_resource_base() {
    let project = VirtualProjectBuilder::new()
        .entry("docs/main.qd").unwrap()
        .add_source("docs/main.qd", ".include {reader} sandbox:{subdocument}").unwrap()
        .add_asset("docs/data.txt", b"caller data".to_vec()).unwrap()
