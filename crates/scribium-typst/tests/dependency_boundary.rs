...
fn production_manifest_has_no_upward_or_native_dependencies() {
    let manifest = fs::read_to_string(Path::new(env!("CARGO_MANIFEST_DIR")).join("Cargo.toml"))
        .expect("read scribium-typst manifest");
    ...
    for dependency in forbidden {
        assert!(
            ...
            "scribium-typst production dependencies must not include {dependency}: {dependencies:?}"
        );
    }
}
    ...
    for source in sources {
        ...
        for needle in forbidden {
            assert!(
                ...
                "{needle} must not appear in pure scribium-typst production source: {}",
                source.display()
            );
        }
    }
