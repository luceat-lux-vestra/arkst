...
pub(crate) fn evaluate(
    ...
    named_args: &[arkst_ir::IrNamedArg],
    has_body: bool,
...
fn append_row_plain_text(row: &arkst_ir::IrTableRow, output: &mut String) -> Option<()> {
...
mod tests {
    ...
    fn named_arg(name: &str, value: IrValue) -> arkst_ir::IrNamedArg {
        arkst_ir::IrNamedArg {
            name: name.to_string(),
            name_span: arkst_source::SourceSpan::new(arkst_source::SourceId(0), 0, 0),
            value,
            span: arkst_source::SourceSpan::new(arkst_source::SourceId(0), 0, 0),
        }
    }
    ...
    fn decimal_numeric_surface_matches_upstream_boundaries() {
        ...
        assert_eq!(
            ...
            .expect("large decimals should not use a Arkst-only limit"),
            number(0.0)
        );
        assert!(evaluate(
            "truncate",
    ...
    fn jdk25_oracle_matches_all_generated_case_mappings() {
        let Ok(path) = std::env::var("ARKST_JDK25_UNICODE_MAPS") else {
            return;
        };
    ...
    fn jdk25_oracle_matches_public_string_builtins() {
        let (Ok(corpus_path), Ok(output_path)) = (
            std::env::var("ARKST_JDK25_UNICODE_CORPUS"),
            std::env::var("ARKST_JDK25_UNICODE_CORPUS_OUTPUT"),
        ) else {
            return;
        };
    ...
    fn string_operations_reject_unsupported_values_and_invalid_bindings() {
        ...
        let rich_content = IrValue::Content(vec![IrNode::Paragraph {
            content: vec![IrInline::Strong {
                content: vec![IrInline::Text {
                    content: "rich".into(),
                    span: arkst_source::SourceSpan::new(arkst_source::SourceId(0), 0, 4),
                }],
                span: arkst_source::SourceSpan::new(arkst_source::SourceId(0), 0, 4),
            }],
            span: arkst_source::SourceSpan::new(arkst_source::SourceId(0), 0, 4),
                ...
                &[
                    IrValue::Content(vec![IrNode::Paragraph {
                        content: vec![IrInline::Strong {
                            content: vec![IrInline::Text {
                                content: "same".into(),
                                span: arkst_source::SourceSpan::new(
                                    arkst_source::SourceId(0),
                                    0,
                                    4,
                                ),
                            }],
