//! AST-to-IR conversion — translates the parsed Markdown AST into the Arkst IR.
//!
//! This is the bridge between `arkst-markdown` (parser output) and `ir`
...
fn lambda_parameters_to_ir(
    header: &arkst_markdown::ast::LambdaHeader,
    source_id: SourceId,
) -> Vec<IrParameter> {
    header
        .parameters
...
fn list_item_to_ir(
    item: &arkst_markdown::ast::ListItem,
    source_id: SourceId,
    diagnostics: &mut Vec<Diagnostic>,
    source_mode: Mode,
    function_body: bool,
...
fn table_row_to_ir(
    row: &arkst_markdown::ast::TableRow,
    source_id: SourceId,
    diagnostics: &mut Vec<Diagnostic>,
    source_mode: Mode,
    function_body: bool,
...
fn call_arguments_to_ir(
    ...
    diagnostics: &mut Vec<Diagnostic>,
    source_mode: Mode,
) -> Option<(Vec<arkst_ir::IrValue>, Vec<IrNamedArg>, Vec<IrCallArgument>)> {
...
fn value_to_ir(
    ...
    diagnostics: &mut Vec<Diagnostic>,
    source_mode: Mode,
) -> Option<arkst_ir::IrValue> {
    match value {
        Value::String(s) => Some(arkst_ir::IrValue::String(s.clone())),
        Value::Number(n) => Some(arkst_ir::IrValue::Number(*n)),
        Value::Boolean(b) => Some(arkst_ir::IrValue::Boolean(*b)),
        Value::Identifier(id) => Some(arkst_ir::IrValue::Identifier(id.clone())),
        Value::Range(range) => Some({
            ...
            arkst_ir::IrValue::Range(IrRange {
        ...
        Value::Lambda {
            ...
            body,
            span,
        } => Some(arkst_ir::IrValue::Callable(arkst_ir::IrCallable {
        ...
        Value::InlineBody {
            ...
            body,
            span,
        } => Some(arkst_ir::IrValue::InlineBody(IrInlineBody {
        ...
        Value::Content(inlines) => Some(arkst_ir::IrValue::Content(content_value_to_ir(
            inlines,
            source_id,
            diagnostics,
            source_mode,
...
fn content_value_to_ir(
    ...
    }] = inlines
    {
        ...
        let span = arkst_source::ByteSpan::new(start.unwrap_or(0), end.unwrap_or(0));
...
fn image_resource_diagnostic(
    ...
    if image_reference_is_local(destination) {
        ...
        Some(Diagnostic {
            ...
            secondary: Vec::new(),
            hints: vec![
                "Use a project-relative local image path; network and absolute resources are not fetched by Arkst."
                    .to_string(),
            ],
        })
    }
...
fn byte_to_source_span(byte_span: &arkst_source::ByteSpan, source_id: SourceId) -> SourceSpan {
    SourceSpan::new(source_id, byte_span.start, byte_span.end)
}

fn raw_body_to_ir(raw_body: &arkst_markdown::ast::RawBody) -> IrRawBody {
    IrRawBody::new(raw_body.source.clone(), raw_body.span)
}
#[cfg(test)]
mod tests {
    use super::*;
    use arkst_markdown::ast::{FrontMatter, TableAlignment, TableCell, TableRow};
    use arkst_source::{ByteSpan, SourceId};
    ...
    fn preserve_call_chain_segments_and_provenance_in_ir() {
        let source = ".a {x}::b {y}\n";
        let document = arkst_markdown::parse_qd(source);
    ...
    fn range_survives_ast_to_ir_as_a_typed_source_backed_value() {
        for (source, expected_start, expected_end, expected_span) in [
            ...
            assert!(matches!(
                ...
                [arkst_ir::IrValue::Range(arkst_ir::IrRange {
                    start,
                    end,
                    span,
    ...
    fn literal_range_endpoint_conversion_is_checked_at_the_signed_boundary() {
        for (source, expected_start, expected_end) in [
            ...
            assert!(matches!(
                positional_args.as_slice(),
                [arkst_ir::IrValue::Range(arkst_ir::IrRange { start, end, .. })]
    ...
    fn range_span_rebasing_is_preserved_from_markdown_ast_to_ir() {
        ...
        fn ir_range_span(document: &arkst_ir::IrDocument) -> SourceSpan {
            for node in &document.nodes {
                match node {
                    IrNode::FunctionCall {
                        positional_args, ..
                    } => {
                        if let Some(arkst_ir::IrValue::Range(range)) = positional_args.first() {
                            return range.span;
                        }
                    }
                    IrNode::Paragraph { content, .. } => {
                        for inline in content {
                            ...
                            {
                                if let Some(arkst_ir::IrValue::Range(range)) =
    ...
    fn convert_unordered_list() {
        use arkst_markdown::ast::ListItem;
        let doc = Document {
    ...
    fn markdown_images_preserve_nested_alt_and_classify_nonlocal_resources() {
        ...
        let document = arkst_markdown::parse_md(source);
        let (ir, diagnostics) = ast_to_ir_with_diagnostics_for_mode(
            &document,
            source_id(),
            &empty_project_metadata(),
        ...
        for destination in [
            ...
            let document = arkst_markdown::parse_md(&format!("![alt]({destination})\n"));
    ...
    fn quarkdown_images_keep_the_existing_unsupported_boundary() {
        let document = arkst_markdown::parse_qd("![alt](image.png)\n");
        let (_, diagnostics) = ast_to_ir_with_diagnostics_for_mode(
            &document,
            source_id(),
            &empty_project_metadata(),
    ...
    fn strikethrough_html_pairs_preserve_del_and_s_tag_identity() {
        ...
        let nested_document = arkst_markdown::parse_md(nested_source);
    ...
    fn markdown_html_comments_are_inline_semantic_noops_and_neutral_for_pairs() {
        ...
        let document = arkst_markdown::parse_md(unknown_source);
        let (ir, diagnostics) = ast_to_ir_with_diagnostics_for_mode(
            &document,
            source_id(),
            &empty_project_metadata(),
    ...
    fn markdown_comment_separator_preserves_distinct_list_and_code_nodes() {
        let lists = arkst_markdown::parse_md("- foo\n- bar\n\n<!-- -->\n\n- baz\n- bim\n");
        ...
        let list_and_code = arkst_markdown::parse_md("- foo\n- bar\n\n<!-- -->\n\n    code\n");
    ...
    fn unsupported_html_keeps_deterministic_diagnostics_and_original_spans() {
        ...
        let inline_document = arkst_markdown::parse_md(inline_source);
        ...
        let block_document = arkst_markdown::parse_md(block_source);
        ...
        let ambiguous_document = arkst_markdown::parse_md(ambiguous_source);
