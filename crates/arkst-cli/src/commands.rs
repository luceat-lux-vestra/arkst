...
use arkst_project::{VirtualPathBuf, VirtualProject, VirtualProjectBuilder};
...
/// compiler into the Arkst process.
...
fn ensure_no_errors(diagnostics: &[arkst_core::Diagnostic]) -> anyhow::Result<()> {
    let error_count = diagnostics
        .iter()
        .filter(|d| matches!(&d.severity, arkst_core::Severity::Error))
        .count();
    if error_count > 0 {
...
fn collect_evidenced_inline_explicit_errors<'a>(
    ...
    for inline in inlines {
        match inline {
            ...
            // #431 established image-alt as a distinct contract: native
            // explicit error is consumed rather than materialized. Any
            // unexpected retained inline error below Image remains fatal.
            IrInline::Image { content, .. } => {
                collect_unevidenced_explicit_errors_from_inlines(
                    content,
...
fn collect_evidenced_nested_explicit_errors<'a>(
    ...
    allow_inline_owners: bool,
) {
    for node in nodes {
        match node {
            IrNode::Heading { content, .. } | IrNode::Paragraph { content, .. }
                if allow_inline_owners =>
            {
                collect_evidenced_inline_explicit_errors(content, collection);
            }
            IrNode::Table { header, rows, .. } if allow_inline_owners => {
                for row in std::iter::once(header).chain(rows) {
                    for cell in &row.cells {
            ...
            IrNode::Component {
                ...
                // Do not generalize Markdown-inline ownership into component
                // trees without separate evidence.
...
fn collect_explicit_error_components<'a>(
    ...
    for node in nodes {
        match node {
            IrNode::Heading { content, .. } | IrNode::Paragraph { content, .. } => {
                collect_evidenced_inline_explicit_errors(content, &mut collection);
            }
            IrNode::Table { header, rows, .. } => {
                for row in std::iter::once(header).chain(rows) {
                    for cell in &row.cells {
                        collect_evidenced_inline_explicit_errors(&cell.content, &mut collection);
                    }
                }
            }
            IrNode::Component {
...
/// `.arkst.{name}.{pid}.{seq:x}.tmp`. The leading dot keeps the file
...
fn temp_candidate_names(parent: &Path, out_path: &Path, start: u64) -> Vec<PathBuf> {
    ...
    (0..MAX_TEMP_CANDIDATES as u64)
        .map(|i| parent.join(format!(".arkst.{}.{}.{:x}.tmp", name, pid, start + i)))
        .collect()
}
...
mod tests {
    ...
    fn ensure_no_errors_fails_on_error() {
        let diagnostics = vec![arkst_core::Diagnostic {
    ...
    fn ensure_no_errors_passes_on_empty() {
        let diagnostics: Vec<arkst_core::Diagnostic> = vec![];
        let result = ensure_no_errors(&diagnostics);
        assert!(result.is_ok());
    }
    ...
    fn symlink_then_dotdot_to_distinct_output_is_allowed() {
        ...
        assert!(
            names
                .iter()
                .all(|n| !n.to_string_lossy().starts_with(".arkst.")),
    ...
    fn temp_candidate_names_follow_the_documented_format() {
        ...
        assert_eq!(
            ...
            format!(".arkst.out.typ.{pid}.0.tmp")
        );
        assert_eq!(
            names[1].file_name().unwrap().to_string_lossy(),
            format!(".arkst.out.typ.{pid}.1.tmp")
        );
        assert_eq!(
            names[names.len() - 1]
                .file_name()
    ...
    fn temp_write_skips_first_candidate_occupied_by_regular_file() {
        ...
        let temps: Vec<_> = fs::read_dir(dir.path())
            ...
            .map(|e| e.unwrap().file_name())
            .filter(|n| {
                n.to_string_lossy().starts_with(".arkst.")
    ...
    fn temp_write_leaves_no_artifacts_after_success_and_failure() {
        ...
        let temps: Vec<_> = fs::read_dir(dir.path())
            .unwrap()
            .map(|e| e.unwrap().file_name())
            .filter(|n| n.to_string_lossy().starts_with(".arkst."))
            .collect();
    ...
    fn successful_build_leaves_no_temp_artifacts_in_output_dir() {
        ...
        // Build to a fresh directory, then reject: no `.arkst.*.tmp`
    ...
    fn root_relative_output_colliding_with_input_is_rejected() {
        ...
        // directory and a `..`: `\A\arkst\arkst\<dir>\new\..\document.qd`.
    ...
    fn root_relative_output_to_distinct_file_is_written() {
        ...
        // `\A\arkst\arkst\<dir>\out.typ` — root-relative, no drive prefix.
        let mut output = PathBuf::from("\\");
