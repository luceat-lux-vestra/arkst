...
#[test]
fn build_keeps_prior_log_observable_when_later_error_fails_compilation() {
    let dir = tempdir().unwrap();
    let input = dir.path().join("main.qd");
    ...
    assert!(stderr.contains("E3011"), "{stderr}");
    assert!(stderr.contains("boom"), "{stderr}");
    assert!(stderr.contains("found 1 error(s)"), "{stderr}");
    assert!(
        !dir.path().join("main.typ").exists(),
        "failed build must not publish an output artifact"
    );
}
