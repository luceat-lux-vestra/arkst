//! Typst lowering — converts the Arkst IR into Typst source code.
//!
...
/// Lower a Arkst IR document to Typst source code.
///
...
/// Lower a Arkst IR document to Typst source code without source map.
...
impl LoweringContext {
    ...
    fn lower_node(&mut self, node: &IrNode) {
        match node {
            ...
            IrNode::ChainedFunctionCall {
                ...
                let before = self.output.len();
                self.push_str("// Arkst parser-preserved call chain: ");
        ...
        match inline {
            ...
            IrInline::Image {
                ...
                if is_local_resource_reference(destination) {
                    ...
                    self.push_str("/* Arkst image resource rejected: ");
    ...
    fn lower_value(&mut self, value: &IrValue) {
        match value {
            ...
            IrValue::Range(_) => {
                ...
                // the backend.
                self.push_str(
                    "panic(\"Arkst typed Range reached Typst lowering without a semantic consumer\")",
                );
            }
            ...
            IrValue::Pair(_) | IrValue::Dictionary(_) => {
                ...
                self.push_str(
                    "panic(\"Arkst structured value reached Typst lowering without a semantic consumer\")",
                );
            }
            ...
            IrValue::Callable(callable) => {
                ...
                let before = self.output.len();
                self.push_str(
                    "/* Arkst callable reached Typst lowering without evaluator consumption */",
                );
            ...
            IrValue::InlineBody(body) => {
                ...
                let before = self.output.len();
                self.push_str(
                    "/* Arkst contextual inline body reached Typst lowering without evaluator consumption */",
                );
            ...
            IrValue::Size(_) | IrValue::Color(_) | IrValue::Enum(_) => {
                ...
                self.push_str(
                    "panic(\"Arkst domain value reached Typst lowering without a semantic consumer\")",
                );
            }
            IrValue::Component(component) => {
                ...
                let before = self.output.len();
                self.push_str(
                    "panic(\"Arkst component reached Typst lowering without a semantic consumer\")",
                );
                if component.span().source_id != SourceId(0) {
                    self.record_span(component.span(), self.output.len() - before);
                }
    ...
    fn lower_list_item(&mut self, marker_prefix: &str, item: &arkst_ir::IrListItem) {
        let before = self.output.len();
...
mod tests {
    ...
    fn lower_pre_evaluation_range_is_explicit_and_never_semantic_none() {
        ...
        let code = super::lower_to_typst_code(&doc);
        assert!(code.contains("panic(\"Arkst typed Range reached Typst lowering"));
    ...
    fn debug_render_parser_preserved_chain_after_semantic_gate() {
        ...
        assert_eq!(
            code,
            "// Arkst parser-preserved call chain: .a::b (unresolved chain; evaluator did not resolve it)\n\n"
        );
        assert_eq!(map.len(), 1);
