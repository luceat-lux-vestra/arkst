...
fn ir_manifest_has_only_allowed_production_dependencies() {
    ...
    assert!(
        dependency_names.contains("scribium-source"),
        "scribium-ir must depend on scribium-source for SourceSpan"
    );
    assert!(dependency_names.contains("serde"));
    assert_eq!(
        dependency_names.len(),
        2,
        "scribium-ir must not acquire upward Scribium production dependencies"
    );
}
