//! Native Typst CLI subprocess adapter for Arkst.
//!
...
//! Pure IR-to-Typst lowering lives in `arkst-typst`.

use arkst_project::VirtualPathBuf;
...
/// stage Arkst source and assets. It is not an OS-wide filesystem or
...
/// Resolves a resource path using Arkst's logical source semantics.
///
...
"###;
    ...
    fn typst_diagnostics_sanitize_native_and_slash_temp_paths() {
        let temporary_root = Path::new("D:\\a\\_temp\\arkst");
        let stderr = "error: D:/a/_temp/arkst/project/docs/main.typ:1:1\n".to_string()
            + "error: \\\\?\\D:\\a\\_temp\\arkst\\project\\docs\\main.typ:2:1\n"
