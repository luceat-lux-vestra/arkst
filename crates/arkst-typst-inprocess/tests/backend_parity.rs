...
//! Arkst compile/lowering path. The subprocess observation receives a
...
use arkst_core::{compile, CompileOptions, VirtualProject, VirtualProjectBuilder};
use arkst_source::{SourceMapEntry, SourceSpan};
use arkst_typst::lowering::lower_to_typst;
...
enum BackendOutcomeExpectation {
    ...
    /// rejects the request at its Arkst-owned World boundary.
...
fn find_logical_path(text: &str, project: &VirtualProject) -> Option<String> {
    ...
    if let Some((_, entry_name)) = project.entry().as_str().rsplit_once('/') {
        ...
        let generated_parent = project
            .entry()
            .parent()
            .unwrap_or_else(arkst_project::VirtualPathBuf::root);
...
fn required_typst() -> Option<PathBuf> {
    let path = if let Some(path) = std::env::var_os("ARKST_TYPST_PATH") {
        ...
        if !path.is_file() {
            return missing_typst("ARKST_TYPST_PATH does not name a file");
        }
        path
...
fn parity_fixtures() -> Vec<ParityFixture> {
    vec![
        lowered_fixture(
            ...
            "# Simple document\n\nA generated Arkst document.\n",
            &[],
            success_expectation(&["Simple document", "generated Arkst"]),
        ),
        lowered_fixture(
            "nested-inline-block",
        ...
        lowered_fixture(
            ...
            &[],
            success_expectation(&["Hello, Arkst", "Hello world"]),
        ),
        lowered_fixture(
            "real-markdown",
            ...
            &[],
            success_expectation(&["Arkst Markdown example", "strong text"]),
        ),
        lowered_fixture(
            "real-gfm",
...
fn lowered_fixture_with_sources(
    ...
    assert!(
        ...
        "fixture '{}' did not compile through Arkst: {:?}",
        name,
        result.diagnostics
    );
    ...
    if mapped {
        ...
        if ambiguous {
            ...
            let other_id = project
                .sources()
                .get_id(
                    &arkst_project::VirtualPathBuf::parse("docs/target/Users/other.qd")
                        .expect("path"),
                )
...
fn font_fixture() -> ParityFixture {
    ...
    // Font selection is not yet a Arkst semantic, so this deterministic
