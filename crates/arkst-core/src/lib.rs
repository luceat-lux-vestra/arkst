mod engine_adapter;
/// `arkst-core` — Arkst's foundational library.
///
/// Responsibilities:
...
// arkst-engine. Evaluator keeps a core wrapper because its legacy public
...
///
/// The implementation is in `arkst-engine`; these small adapters preserve
...
pub mod ast_to_ir {
    ...
    use arkst_diagnostics::Diagnostic;
    use arkst_ir::IrDocument;
    use arkst_markdown::ast::Document;
    use arkst_source::SourceId;
    ...
    pub fn ast_to_ir_with_diagnostics(
        ...
        arkst_engine::ast_to_ir::ast_to_ir_with_diagnostics(
            doc,
            source_id,
    ...
    pub(crate) fn ast_to_ir_with_diagnostics_for_mode(
        ...
        arkst_engine::ast_to_ir::ast_to_ir_with_diagnostics_for_mode(
            doc,
            source_id,
            &engine_adapter::document_metadata_defaults(project_metadata),
            match source_mode {
...
pub use arkst_project::virtual_project;

// Compatibility facade: implementation ownership lives in arkst-compat.
pub use arkst_compat as compatibility;
// Compatibility facade: implementation ownership lives in arkst-diagnostics.
pub use arkst_diagnostics as diagnostics;
...
fn source_mode_for_entry(entry: &arkst_project::VirtualPathBuf) -> SourceMode {
...
pub fn compile_with_capabilities(
    ...
    let parsed = match source_mode {
        SourceMode::Markdown => {
            arkst_markdown::parse_with_mode(source, arkst_markdown::Mode::Markdown)
        }
        SourceMode::Quarkdown => {
            arkst_markdown::parse_with_mode(source, arkst_markdown::Mode::Quarkdown)
        }
    };
    ...
    let mut diagnostics: Vec<Diagnostic> = parsed
        ...
        .map(|d| Diagnostic {
            ...
            severity: Severity::Error,
            message: d.message,
            primary: Some(arkst_source::SourceSpan {
                source_id,
                start: d.span.start,
                end: d.span.end,
            }),
...
mod tests {
    ...
    fn compile_rejects_positional_after_named_through_shared_binder() {
        for source in [
            ...
            assert_eq!(
                ...
                vec![arkst_source::SourceSpan::new(
                    source_id,
                    named_start,
                    named_end,
                )],
    ...
    fn compile_evaluates_block_and_inline_chain_value_flow() {
        ...
        assert_eq!(
            first_span,
            arkst_source::SourceSpan::new(source_id, 0, source.find('\n').unwrap())
        );
...
Yiddish"
    ...
    fn doclang_preserves_v260_english_name_and_replaces_previous_locale() {
        ...
        let (result, _) = compile_source(source);
        assert!(result.diagnostics.is_empty(), "{result:?}");
        assert_eq!(output_text(&result), "Italian\nFrench (Canada)");
        assert_eq!(
            result.ir.metadata.document_state.locale,
            Some(crate::ir::IrDocumentLocale {
                tag: "fr-CA".to_string(),
                localized_name: "French (Canada)".to_string(),
            })
        );
    }
    ...
    fn doclang_v260_rejects_unavailable_and_jdk_only_identifiers_atomically() {
        for invalid in ["xx-YY", "en_US", "   ", "English (United States, Computer)"] {
            let source = format!(
                ".doclang {{en}}
.doclang {{{invalid}}}
.doclang
...
.if {.startswith {Hello} {he} ignorecase:{yes}}\n\
    ...
    fn compile_implicit_parameter_diagnostic_preserves_utf8_and_crlf_span() {
        ...
        assert_eq!(
            result.diagnostics[0].primary,
            Some(arkst_source::SourceSpan::new(source_id, start, start + 2))
        );
    }
...
.ifpresent {inner} {@lambda value: .value}\n";
    ...
    fn optional_parameter_spans_survive_utf8_and_crlf_frontend_to_ir_conversion() {
        ...
        let source_id = project
            ...
            .expect("source id");
        let parsed = arkst_markdown::parse_with_diagnostics(source);
    ...
    fn compile_reports_unimplemented_chain_callees_with_specific_spans() {
        for source in [".a::b\n", ".a::b::c\n", ".a {x}::b {y}\n"] {
            let parsed = arkst_markdown::parse_qd(source);
            let arkst_markdown::ast::Block::DirectiveCall { chain, .. } = &parsed.nodes[0] else {
            ...
            assert_eq!(
                diagnostic.primary,
                Some(arkst_source::SourceSpan::new(source_id, 0, 2))
            );
            assert!(result.ir.nodes.is_empty());
        }
    }
    ...
    fn compile_reports_chain_failures_in_inline_and_content_paths() {
        ...
        let parsed = arkst_markdown::parse_qd(inline_source);
        let arkst_markdown::ast::Block::Paragraph { content, .. } = &parsed.nodes[0] else {
            panic!("expected inline paragraph");
        };
        assert!(content.iter().any(|inline| matches!(
            inline,
            arkst_markdown::ast::Inline::DirectiveCall { chain, .. }
                if !chain.is_empty()
        )));
        ...
        let parsed = arkst_markdown::parse_qd(content_source);
        let arkst_markdown::ast::Block::DirectiveCall { arguments, .. } = &parsed.nodes[0] else {
            panic!("expected outer call");
        };
        let Some(arkst_markdown::ast::CallArgument::Positional {
            value: arkst_markdown::ast::Value::Content(content),
            ..
        }) = arguments.first()
        else {
    ...
    fn compile_md_preserves_utf8_crlf_break_semantics_and_spans() {
        ...
        match content.as_slice() {
            [IrInline::Text {
                ...
                assert_eq!(second, "다음");
                assert_eq!(third, "끝");
                assert_eq!(*first_span, arkst_source::SourceSpan::new(source_id, 0, 6));
                assert_eq!(*soft_span, arkst_source::SourceSpan::new(source_id, 6, 8));
                assert_eq!(
                    *second_span,
                    arkst_source::SourceSpan::new(source_id, 8, 14)
                );
                assert_eq!(*hard_span, arkst_source::SourceSpan::new(source_id, 14, 18));
                assert_eq!(
                    *third_span,
                    arkst_source::SourceSpan::new(source_id, 18, 21)
                );
            }
    ...
    fn compile_variable_declaration_and_reference() {
        let (result, _) = compile_source(".var {name} {Arkst}\nHello .name\n");
        assert!(result.diagnostics.is_empty());
        assert_eq!(result.ir.nodes.len(), 1);
        let IrNode::Paragraph { content, .. } = &result.ir.nodes[0] else {
            panic!()
