...
fn stacked(result: &arkst_core::CompileResult) -> &arkst_core::ir::IrStackedComponent {
    let [IrNode::Component {
        component: arkst_core::ir::IrComponent::Stacked(component),
...
fn body_is_lazy_and_nested_failures_do_not_publish_outer_components() {
    ...
    assert!(
        matches!(content.first(), Some(arkst_core::ir::IrInline::Text { content, .. }) if content == "before")
    );
