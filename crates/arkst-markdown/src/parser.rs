...
use arkst_quarkdown::{Arg, ArgContent, QuarkdownCall, Value as QuarkdownValue};
use arkst_source::{ByteSpan, SourceText};
use rushdown::ast::{
...
impl BlockParser for QuarkdownBlockParser {
    ...
    fn cont(
        ...
        if as_extension_data!(arena, node_ref, QuarkdownBlock).header_pending {
            ...
            let mut header_became_body = false;
            ...
            match arkst_quarkdown::parse_call(candidate) {
                Ok(Some((call, _))) => {
                    ...
                    if has_continuation && has_following_input {
                        ...
                        .is_some_and(arkst_quarkdown::has_trailing_continuation)
                        && !is_blank(&line)
                        && body_line_prefix(&line, source, segment, call_start).is_some()
                    {
                        let block = as_extension_data_mut!(arena, node_ref, QuarkdownBlock);
                        block.call = Segment::new(call_start, call_end);
                        block.raw_body_start = body_start;
                        ...
                        block.header_pending = false;
                        block.continuation_pending = false;
                        header_became_body = true;
                    } else if source
                        .get(call_start..call_end)
                        .is_some_and(arkst_quarkdown::has_trailing_continuation)
                    {
                        // A valid root-level trailing continuation ends the
                        // header before this unrelated remainder. Finalize
                        // the call but leave the current physical line for
                        // Rushdown to parse as a separate block.
                        let block = as_extension_data_mut!(arena, node_ref, QuarkdownBlock);
                        block.call = Segment::new(call_start, call_end);
                        block.raw_body_start = body_start;
                        block.raw_body_end = call_end;
                ...
                Err(error)
                    if error.code == "E2003"
                        || (continuation_pending
                            && arkst_quarkdown::has_trailing_continuation(candidate)) => {}
                _ => return None,
            }
            if !header_became_body {
                reader.advance_to_eol();
                return Some(State::HAS_CHILDREN);
            }
        }
...
impl InlineParser for QuarkdownInlineParser {
    ...
    fn parse(
        ...
        let source = reader.source();
        let parsed = arkst_quarkdown::parse_inline_call(source, segment.start());
...
impl InlineParser for QuarkdownTightInlineParser {
    ...
    fn parse(
        ...
        let (_, segment) = reader.peek_line_bytes()?;
        let source = reader.source();
        let (call, end) = match arkst_quarkdown::parse_tight_call(source, segment.start()) {
...
fn contextualize_lambda_body(
    ...
    let header = match arkst_quarkdown::parse_lambda_header(source, first_line) {
        Ok(header) => header,
        Err(error) => {
            diagnostics.push(ParserDiagnostic {
                code: error.code,
...
fn convert_block(
    ...
    match arena[node].kind_data() {
        ...
        KindData::Extension(_) if matches_extension_kind!(arena, node, QuarkdownBlock) => {
            ...
            match arkst_quarkdown::parse_call(source.get(call_span.start..call_span.end)?) {
...
/// Arkst AST. The source span remains the original byte range. The
    ...
    match arena[node].kind_data() {
        ...
        KindData::Extension(_) if matches_extension_kind!(arena, node, QuarkdownInline) => {
            ...
            let parsed = if source.as_bytes().get(call_span.start) == Some(&b'{') {
                arkst_quarkdown::parse_tight_call(source, call_span.start)
            } else {
                arkst_quarkdown::parse_inline_call(source, call_span.start)
            };
...
fn convert_call_segment(
    segment: &arkst_quarkdown::CallSegment,
    source: &str,
    base: usize,
    call_base: usize,
    diagnostics: &mut Vec<ParserDiagnostic>,
...
fn convert_call_arguments(
    arguments: &[arkst_quarkdown::CallArgument],
    call_name: &str,
    source: &str,
    base: usize,
    call_base: usize,
    ...
    arguments
        ...
        .map(|argument| match argument {
            ...
            arkst_quarkdown::CallArgument::Named(arg) => CallArgument::Named(convert_named_arg(
                arg,
                source,
                base,
                call_base,
