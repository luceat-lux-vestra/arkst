use arkst_markdown::ast::{Block, CallArgument, Document, Inline, ListItem};
use arkst_markdown::{parse_md, parse_qd, parse_with_mode, Mode};
            ...
            for argument in arguments {
                match argument {
                    CallArgument::Positional { value, span } => {
                        assert_valid_span(*span, source);
                        if let arkst_markdown::ast::Value::Content(content) = value {
                            for inline in content {
                                assert_inline_spans(inline, source);
                            }
                        }
                    ...
                    CallArgument::Named(argument) => {
                        ...
                        if let arkst_markdown::ast::Value::Content(content) = &argument.value {
                            for inline in content {
                                assert_inline_spans(inline, source);
                            }
                        }
...
fn qd251_logical_comparison_expression_remains_structural_and_source_backed() {
    ...
    let Some(CallArgument::Positional {
        value: arkst_markdown::ast::Value::Content(condition),
        ..
    }) = arguments.first()
    else {
...
fn qd251_plaintext_keeps_existing_scalar_and_content_argument_classification() {
    for (source, expect_content) in [
        ...
        assert!(
            output.diagnostics.is_empty(),
            "unexpected diagnostics after source-backed content conversion: {output:?}"
        );
        ...
        assert_eq!(
            matches!(argument, arkst_markdown::ast::Value::Content(_)),
            expect_content
        );
    }
}
