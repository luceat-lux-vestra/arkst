...
`DynamicValue` text. Scribium now retains the source-backed, indentation-
normalized body alongside the parsed `CallBody`; the bounded setter consumes
that text for the final `codeBlocks` conversion without evaluating nested body
calls. Invalid raw enum text still fails before state publication. Complete
body fallback coverage for the remaining layout/document consumers and
caption rendering remains open.
