...
| Bounded semantic evidence (Unicode string semantics) | `.capitalize`, `.startswith` | `.capitalize` reproduces Kotlin `Char.titlecase()` over pinned JDK 17-compatible Unicode 13.0 full/simple mappings, including the `ᾀ` -> `Ἀι` multi-scalar result, and `.startswith(ignorecase:true)` uses the corresponding Unicode 13 simple upper/lower mappings without normalization or whole-string case conversion. Independent `ǳ`/`ǲ`, `ŉ`, `ß`, Greek `ς`/`Σ`, long-s, dotted-I, decomposed-prefix, and post-Unicode-13 `Ꟑ`/`ꟑ` cases are covered; #172 closes this bounded string-semantics slice at `SUPPORTED_SEMANTICS`. |
