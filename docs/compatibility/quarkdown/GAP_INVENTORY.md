...
- **Arkst comparison baseline:** `7144683346fd6e39c49ef0923733c856a6a55f42`
...
relevant output boundary and is covered by Arkst tests. The inventory uses
...
rows are `PARTIAL`. `.localization`/`.localize` are canonical #151-owned
`SUPPORTED_SEMANTICS` general stdlib rows implemented by
[#196](https://github.com/luceat-lux-vestra/arkst/issues/196); the #152 manifest
retains them only as `NOT_APPLICABLE` ownership handoffs. `.get` is now a
bounded `SUPPORTED_SEMANTICS` evaluator operation owned by
[#194](https://github.com/luceat-lux-vestra/arkst/issues/194). Bounded library
inspection is now `PARTIAL` under
[#195](https://github.com/luceat-lux-vestra/arkst/issues/195). Bounded logger/diagnostic implementation is complete under
[#197](https://github.com/luceat-lux-vestra/arkst/issues/197); the canonical
rows remain `PARTIAL` only for the cross-owned or intentionally non-portable
boundaries documented below. The broader
...
- **Unsupported:** the surface is known and may parse/preserve, but Arkst
  does not provide its semantics.
- **Arkst extension:** Arkst behavior without a corresponding public
...
|---|---|
| Supported at the stated bounded boundary | Logical and comparison operations; optionality callbacks; Collection selector sorting; typed Dictionary `.get` lookup; bounded localization table mutation and `.localize` lookup; the VirtualProject/resource model as `SUPPORTED_SEMANTICS`; argumentless inline `.br`; bounded `.docauthor`/`.docauthors`; block-only Stacked row/column/grid consumers |
| Partially compatible | Function declarations/calls; variables; conditionals; lambdas/callables; iteration; collections; Dictionary/Pair/Range consumers outside the bounded `.get` lookup; type/value conversion; strings/text; bounded library inspection; bounded logger/diagnostic semantics; metadata/document setup; bounded project data/file identity/resource parsing (`.filename`, `.listfiles`, raw `.csv` parser); complete public component/style/layout surface; complete error taxonomy |
| Compatible at the evidenced bounded boundary | Mathematics/numeric operations |
| Unsupported | Layout/document functions outside the reviewed Stacked, Container, and Landscape slices; remaining unimplemented #189 bibliography family `.bibliography`; public `.csv` callable/table production remains #183-owned rather than classified as a completed #189 surface; #154 `.match`, `.loremipsum`, and `.subdocumentgraph` pending their explicit owner/blocker |
| Arkst extension | `.map` and `.filter` collection transforms; they are tested Arkst behavior but are not v2.5.1 upstream features |
| Intentionally deferred | Remaining `.bibliography` resource/output work, public `.csv` callable/table semantics beyond the bounded raw parser, `.llmstxt`, function-driven metadata, remaining container style/layout families, `.css`/`.cssproperties` until a target-specific HTML backend/product contract exists, arbitrary comparator syntax not present in v2.5.1, and generalized DynamicValue conversion |
...
It starts from Arkst commit 7144683346fd6e39c49ef0923733c856a6a55f42 and
...
Arkst head. Names below are the public `@QFunction` names, including
...
| Bounded semantic evidence (Unicode string semantics) | `.capitalize`, `.startswith` | `.capitalize` reproduces Kotlin `Char.titlecase()` over full/simple mappings generated from the pinned Eclipse Temurin `25.0.4.1+1` oracle (Unicode 16.0), including the `ᾀ` -> `Ἀι` multi-scalar result, and `.startswith(ignorecase:true)` uses the corresponding Kotlin/JVM simple upper/lower mappings without normalization or whole-string case conversion. Independent `ǳ`/`ǲ`, `ŉ`, `ß`, Greek `ς`/`Σ`, long-s, dotted-I, decomposed-prefix, supplementary, and newly mapped Unicode 14–16 cases are covered; #172 closes this bounded string-semantics slice at `SUPPORTED_SEMANTICS`. |
| Bounded semantic evidence (typed dictionary lookup) | `.get` | The dedicated evaluator-native lookup uses the shared `dictionary`, `key`, and optional `orelse` binding slots, canonical String conversion, exact ordered key equality, typed stored/fallback values, and `IrValue::None` for an omitted fallback. Malformed internal keys fail closed with source-backed diagnostics; this is `SUPPORTED_SEMANTICS`, not renderer/output-equivalent support. |
| Bounded semantic evidence (localization tables and lookup) | `.localization`, `.localize` | #196 adds one evaluator-native localization owner over deterministic table state shared by ordinary document scopes. It validates typed Dictionary/String/Boolean values and all locales before atomic merge/replacement, seeds the independently evidenced non-empty `std` table, and splits the requested key only at the first separator boundary, then performs exact table/locale lookup from the current `.doclang` locale (with no locale fallback) and pinned whole-string Kotlin/JVM lowercase-first-then-original key lookup (`entries[key.lowercase()] ?: entries[key]`, including contextual Final_Sigma and not general case-insensitive matching). This is `SUPPORTED_SEMANTICS`, not renderer/output-equivalent or exhaustive resource-data support. |
| Bounded semantic evidence (library inspection) | `.libexists`, `.functionexists`, `.libraries`, `.libfunctions` | #195 implements one ordered evaluator-owned in-memory registry, exact/case-sensitive existence checks, source-function pseudo-libraries, known/unknown behavior, source-backed malformed-argument diagnostics, and transactional rollback without provider/host discovery. These rows are `PARTIAL`, because Quarkdown v2.6 exposes its complete registered stdlib while Arkst deliberately exposes only callable functions rather than fabricating support. |
| Bounded semantic evidence (logger/diagnostics) | `.log`, `.debug`, `.error` | #197 implements shared binding, an explicit platform-neutral `LogSink`, deterministic no-sink behavior, source-backed ordered events, resource/library + sink composition, source-defined shadowing, and structured `.error` diagnostics. #368/PR #371 models the independently evidenced Unit result category. Clean-room PR #378 adds the initial logger-only String subset; closed/unmerged #390 extends Range/Pair/Dictionary evidence, and closed/unmerged #395 confirms recursive Pair projection across left/right/balanced nesting through depth 4 with Range/None members. Closed/unmerged #403 proves that Collection origin is semantically relevant, and closed/unmerged #405 proves the direct-list logger projection is source-spelling-sensitive. Arkst retains the source-backed `trimIndent().trimEnd()` list body only as evaluator-local metadata for the same-scope direct variable-reference boundary, preserving the #405-evidenced unordered marker/spacing, ordered numbering, strong/emphasis spelling, one nested unordered-list level, task markers, and multiline continuation; operation-generated Collections remain fail-closed because their observed JVM projections differ and may contain unstable runtime context identities. Clean-room PRs #382/#385/#386 establish top-level/default/strict `.error` behavior. Closed/unmerged #392/#393 establish one enclosing Container-family / unordered-list / blockquote output context, and merged PR #398 adds independently sourced `.error` spelling for the rendered source echo. Closed/unmerged clean-room #401 extends exact v2.5.1/v2.6.0 evidence to top-level inline, ordered-list, Stacked row/column/grid, Landscape, and a representative deeper `.center` → `.row` composition; closed/unmerged #431 adds direct heading/table-cell and emphasis/strong/strikethrough/link-text owners, and #433 confirms representative composition across structural and inline owners. Exact v2.5.1/v2.6.0 source review shows `FunctionCallNode` error replacement and recursive `NestableNode` child traversal without an owner-depth counter, so Arkst composes the already-supported rendered-owner tree recursively without an artificial depth cap while retaining exact 1:1 E3011 ↔ error-carrier span/message/source-echo pairing. Value contexts, orphan components, and unrelated diagnostics remain fail-closed where no compatibility claim is made. Closure review found no additional #197-owned public error owner: upstream `Numbered` uses a distinct late-evaluation error path, but Arkst `.numbered` is #181-owned; image/image-alt behavior is #182-owned; and public WASM/embedder exposure is #191-owned. Operation-generated Collection projections are an explicitly bounded fail-closed, non-portable JVM boundary rather than a production completion target. #197 is complete as the logger/diagnostic implementation owner, while the canonical rows remain `PARTIAL` for those cross-owned or intentionally non-portable boundaries. Exact Quarkdown HTML/CSS styling is not a Typst/PDF closure requirement. |
...
| Partially implemented | `.autopagebreak`, `.currentpage`, `.footer`, `.formatpagenumber`, `.lastheading`, `.marker`, `.navigation`, `.noautopagebreak`, `.nonumbering`, `.numbering`, `.pageformat`, `.pagemargin`, `.resetpagenumber`, `.tableofcontents`, `.texmacro`, `.totalpages` | Project/front-matter or bounded IR state covers only part of these contracts; remaining function-driven document context and observable layout behavior retain their explicit owners. |
| Partially implemented (bounded font state) | `.font` | Ordered document font layers currently preserve only optional typed `size`. Explicit non-None `main`/`heading`/`code` family arguments fail closed until system/file/URL/Google classification and media/resource registration are implemented. Atomic rollback, callable sharing, source-defined shadowing, and serde are covered; renderer lowering remains open under #175. |
| Partially implemented (bounded document state) | `.paragraphstyle` | Shared numeric conversion plus evaluator-owned partial merge state preserves omission/None inheritance, atomic rollback, callable visibility, source-defined shadowing, and serde. Locale-sensitive defaults and rendered paragraph output remain open under #175. |
| Partially implemented (bounded evaluator) | `.extend`, `.super` | Source-defined callables and regular scalar builtins resolve through the existing evaluator and shared binder/conversion path. Target parameter names, conditional delegation, chained parent links, nested dynamic `.super` inheritance/replacement, scope-local link lifetime, source-backed failures, and #167 savepoint rollback are independently tested in `crates/arkst-core/tests/quarkdown_function_extension.rs`. Specialized native owners, complete failure-effect parity, and rendered output remain outside this evaluator-only slice. |
| Unsupported | `.allemojis`, `.bibliography`, `.box`, `.cite`, `.clip`, `.code`, `.codespan`, `.collapse`, `.emoji`, `.figure`, `.filetree`, `.float`, `.fragment`, `.fullspan`, `.heading`, `.icon`, `.image`, `.keybinding`, `.loremipsum`, `.math`, `.match`, `.mermaid`, `.numbered`, `.pagebreak`, `.paragraph`, `.ref`, `.slides`, `.speakernote`, `.subdocumentgraph`, `.table`, `.tablebyrows`, `.tablecolumn`, `.tablecolumns`, `.tablecompute`, `.tablefilter`, `.tablesort`, `.text`, `.textcollapse`, `.todo`, `.xychart` | The call may be preserved or parsed, but no approved evaluator/backend-neutral semantic implementation exists. `.text`, `.codespan`, `.clip`, `.float`, `.fullspan`, and the remaining component/layout families remain separate compatibility work; the bounded `.row`/`.column`/`.grid`, `.center`, `.align`, `.landscape`, and `.container` slices are classified above. |
| Unsupported (explicit ownership) | `.node` | Public `Flow.kt` `@QFunction`; #150 records only its programmable value-to-content and lazy-body boundary, while Node/Markdown representation, materialization, and backend/output fidelity belong to #154. No Arkst `.node` evaluator path or conformance claim exists. |
| Historical #151 defer/handoff seed (not current status) | `.csv`, `.css`, `.cssproperties`, `.env`, `.filename`, `.htmloptions`, `.includeall`, `.listfiles`, `.llmstxt`, `.pathtoroot`, `.subdocument` | This row preserves the original family-level routing only. It is not a current per-surface classification: `.filename`, `.listfiles`, and the raw `.csv` resource/parser boundary are bounded `PARTIAL` rows under canonical #155/#189, `.env` is a bounded `PARTIAL` deterministic external-input row under #155/#190 with public WASM exposure left to #191, and the other names must be read from the current #151/#154/#155 manifests and active owners rather than inferred as uniformly deferred. |
| Arkst extension | `.map`, `.filter` | Existing typed collection transforms have no corresponding public v2.5.1 `Collection.kt` declarations and are excluded from upstream compatibility counts. |
...
### `.captionposition` block-body fallback
...
`DynamicValue` text after `trimIndent().trimEnd()`. Arkst retains one shared
...
| Family | v2.5.1 public surface and evidence | Arkst status and semantic gap | Test/conformance evidence | Milestone and order |
|---|---|---|---|---|
...
Generic Dynamic String → Markdown conversion outside the explicit `.plaintext`
target remains a documented gap because upstream may reparse Markdown-bearing strings. The
...
Arkst implements this bounded family through `IrValue::None` and the
...
failure-atomicity coverage is in `crates/arkst-core/src/lib.rs::tests::compile_optionality_*`.
...
- Frontend preserves `.if {.islower {2} than:{3}}` as a nested directive with
  ...
  `crates/arkst-markdown/tests/quarkdown_v2_5_1.rs::qd251_logical_comparison_expression_remains_structural_and_source_backed`.
- Unit evaluation covers strict/inclusive ordering, boolean negation, typed
  ...
  rejection: `crates/arkst-engine/src/builtins.rs::tests::*`.
- Unit evaluation covers the decimal surface, strict integral `decimals`,
  ...
  rejection: `crates/arkst-engine/src/builtins.rs::tests::decimal_numeric_surface_matches_upstream_boundaries`.
...
- Integration coverage verifies ordinary/named/chain equivalence, typed
  ...
  partial enclosing output: `crates/arkst-core/src/lib.rs::tests::compile_v251_numeric_decimal_fixture_preserves_typed_value_flow`,
...
- Transcendental unit coverage fixes representative `to_bits()` results for
  ...
  `crates/arkst-engine/src/builtins.rs::tests::transcendental_numeric_surface_matches_upstream_boundaries`
...
- Integration coverage verifies the rendered oracle cases, typed flow, one
  ...
  failure: `crates/arkst-core/src/lib.rs::tests::compile_v251_numeric_transcendental_fixture_preserves_typed_value_flow`
...
- The prior arithmetic/unary regression remains covered by
  `crates/arkst-engine/src/builtins.rs::tests::numeric_*`,
...
- Nested numeric failure is source-backed, emits one diagnostic, and publishes
  no partial output: `crates/arkst-core/src/lib.rs::tests::compile_numeric_nested_failure_is_atomic_and_source_backed`.
- Full compile coverage covers nested calls, variables, user functions,
  ...
  deterministic repeated execution: `crates/arkst-core/src/lib.rs::tests::compile_logical_*`.
