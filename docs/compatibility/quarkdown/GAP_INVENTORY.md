...
| Layout and document functions | `.row`, `.column`, `.grid`, `.container`, `.align`, `.box`, `.figure`, page breaks, tables, and related layout primitives. Evidence: [`Layout.kt`](https://raw.githubusercontent.com/iamgio/quarkdown/v2.5.1/quarkdown-stdlib/src/main/kotlin/com/quarkdown/stdlib/Layout.kt), [`Primitives.kt`](https://raw.githubusercontent.com/iamgio/quarkdown/v2.5.1/quarkdown-stdlib/src/main/kotlin/com/quarkdown/stdlib/Primitives.kt). | **Implemented for the reviewed block-body Stacked slice.** `.row`, `.column`, and `.grid` bind typed arguments, validate before lazy body evaluation, preserve structured children, materialize as typed block nodes, and lower through the Typst backend. `container`, `align`, `box`, `figure`, page breaks, tables, and other layout primitives remain deferred. | `crates/scribium-core/tests/quarkdown_stacked_layout.rs`; `crates/scribium-typst/tests/backend_integration.rs`; `examples/stacked/main.qd`. | Stacked layout family implemented for the reviewed block-body boundary; general String → Markdown conversion, inline Stacked insertion, and other layout families remain deferred. |
...
- `container` / `align` and the remaining layout/component families.

## Selection record

The selected slice is the Stacked layout family: `.row`, `.column`, and
`.grid`, integrated with the existing typed component value foundation and
evaluator-owned value/materialization boundary.
...
deferred candidate is the `container` / `align` family after the remaining #61
semantic gap inventory is re-reviewed.
