...
| Bounded semantic evidence (logger/diagnostics) | `.log`, `.debug`, `.error` | #197 implements shared binding, an explicit platform-neutral `LogSink`, deterministic no-sink behavior, source-backed ordered events, resource/library + sink composition, source-defined shadowing, and structured `.error` failure diagnostics. #368/PR #371 models the independently evidenced Unit result category. Clean-room PR #378 adds the logger-only String subset observed identically on exact Quarkdown v2.5.1/v2.6.0 artifacts: Unit -> `kotlin.Unit`, None -> `None`, closed Range -> `start..end`, and the evidenced plain-text Pair JVM representation. The generic scalar String adapter remains unchanged. Native `arkst build` now supplies an explicit host-owned stdout sink for `Log` events while keeping `Debug` silent; ordinary core compilation remains no-sink/fail-closed. The rows stay `PARTIAL`: unreviewed logger String categories, `.error` rendered/stderr/strict behavior, and Arkst-specific analysis-command logger output policy remain #197 residuals; public WASM/embedder exposure remains deferred under #191. #190's deterministic environment-input boundary is complete and is no longer a logger blocker. |
