...
|---|---|---|
| Implemented (bounded) | `.abs`, `.average`, `.capitalize`, `.concatenate`, `.cos`, `.dictionary`, `.distinct`, `.divide`, `.docdescription`, `.docname`, `.equals`, `.first`, `.foreach`, `.function`, `.getat`, `.groupvalues`, `.html`, `.if`, `.ifnot`, `.ifpresent`, `.include`, `.isempty`, `.isgreater`, `.iseven`, `.isnone`, `.islower`, `.isnotempty`, `.json`, `.last`, `.lowercase`, `.let`, `.markdown`, `.multiply`, `.negate`, `.none`, `.not`, `.otherwise`, `.pair`, `.pi`, `.plaintext`, `.range`, `.read`, `.repeat`, `.rem`, `.reversed`, `.round`, `.second`, `.sin`, `.size`, `.sorted`, `.sqrt`, `.startswith`, `.string`, `.subtract`, `.sum`, `.sumall`, `.takeif`, `.tan`, `.third`, `.truncate`, `.uppercase`, `.var` | Typed evaluator results, shared callable/iterable paths, bounded native-content/resource boundaries, or existing structural document semantics. `.docname` and `.docdescription` use evaluator-owned document state and a final serializable snapshot; `.sorted` is natural-order or `by` selector sorting, not a two-value comparator API. |
| Partially implemented | `.autopagebreak`, `.captionposition`, `.currentpage`, `.docauthor`, `.docauthors`, `.dockeywords`, `.doclang`, `.doctype`, `.font`, `.footer`, `.formatpagenumber`, `.lastheading`, `.marker`, `.navigation`, `.noautopagebreak`, `.nonumbering`, `.numbering`, `.pageformat`, `.pagemargin`, `.paragraphstyle`, `.resetpagenumber`, `.tableofcontents`, `.texmacro`, `.theme`, `.totalpages` | Project/front-matter or IR metadata provides only a different, partial boundary; remaining function-driven document context and observable layout state are not implemented. |
...
| Metadata and document setup | `.doctype`, `.docname`, `.docdescription`, `.docauthor(s)`, `.dockeywords`, `.doclang`, `.theme`, page/paragraph metadata, numbering, and related document state. Evidence: [document metadata](https://quarkdown.com/wiki/document-metadata/), [`Document.kt`](https://raw.githubusercontent.com/iamgio/quarkdown/v2.5.1/quarkdown-stdlib/src/main/kotlin/com/quarkdown/stdlib/Document.kt). | **Partially compatible with an implemented bounded foundation.** `.docname` and `.docdescription` implement read/write semantics through evaluator-owned shared state and a final immutable IR snapshot. `.docname` rejects blank writes and both fields use the existing bounded invocation String boundary. `.doctype`, `.docauthor(s)`, `.dockeywords`, `.doclang`, `.theme`, and observable layout metadata remain deferred. | `crates/scribium-core/src/lib.rs::tests::document_state_*`; `crates/scribium-core/src/ir.rs::tests::document_state_roundtrips_deterministically_and_defaults_for_old_ir`; blank-write provenance and atomicity regression. | DocumentState foundation: implemented; `.docname`: implemented; `.docdescription`: implemented. Remaining document fields are deferred. |
