...
| Bounded semantic evidence (logger/diagnostics) | `.log`, `.debug`, `.error` | #197 implements shared binding/scalar String conversion, an explicit platform-neutral `LogSink`, deterministic no-sink behavior, source-backed ordered events, resource/library + sink composition, source-defined shadowing, and structured `.error` failure diagnostics. The rows are `PARTIAL`: upstream value-context `.log`/`.debug` results materialize as `kotlin.Unit`, richer DynamicValue-to-String formatting is not represented, and normal CLI stdout, Quarkdown's rendered error component, strict-mode exit behavior, and public WASM/embedder exposure remain outside this evaluator slice. #197 remains open for its owned return/value residuals. |
