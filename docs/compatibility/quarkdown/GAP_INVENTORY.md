...
| Metadata and document setup | `.doctype`, `.docname`, `.docdescription`, `.docauthor(s)`, `.dockeywords`, `.doclang`, `.theme`, page/paragraph metadata, numbering, and related document state. Evidence: [document metadata](https://quarkdown.com/wiki/document-metadata/), [`Document.kt`](https://raw.githubusercontent.com/iamgio/quarkdown/v2.5.1/quarkdown-stdlib/src/main/kotlin/com/quarkdown/stdlib/Document.kt). | **Partially compatible with an implemented bounded foundation.** `.docname`, `.docdescription`, `.doctype`, `.docauthor`, and `.docauthors` implement bounded read/write semantics through evaluator-owned shared state and a final immutable IR snapshot. `.docauthor` appends minimal records and reads the first name. `.docauthors` accepts nested list/typed dictionary forms, appends validated authors with ordered scalar info, returns an ordinary typed dictionary, and applies first-slot/last-value duplicate handling. `.doctype` uses the closed `DocumentType` pool (`plain`, `paged`, `slides`, `docs`), returns lowercase names on read, returns no value on write, and leaves state unchanged on invalid or static-text conversion. `.docname` rejects blank writes. `.dockeywords`, `.doclang`, `.theme`, rendering, and observable layout metadata remain deferred. | `crates/scribium-core/src/lib.rs::tests::document_state_*`; `docauthor_*` and `docauthors_*` tests for getter, append order, typed nested info, named/body binding, callable sharing, duplicate handling, shadowing, atomicity, and source spans; `crates/scribium-ir/src/lib.rs::tests::document_state_roundtrips_deterministically_and_defaults_for_old_ir`; independent `docauthor-family` and `docauthors-family` fixtures. | DocumentState foundation: implemented; `.docname`, `.docdescription`, `.doctype`, `.docauthor`, and `.docauthors` at bounded evidenced boundaries: implemented. Remaining document fields are deferred. |
