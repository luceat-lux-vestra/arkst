...
| Bounded semantic evidence (logger/diagnostics) | `.log`, `.debug`, `.error` | #197 implements shared binding, an explicit platform-neutral `LogSink`, deterministic no-sink behavior, source-backed ordered events, resource/library + sink composition, source-defined shadowing, and structured `.error` diagnostics. #368/PR #371 models the independently evidenced Unit result category. Clean-room PR #378 adds the initial logger-only String subset; closed/unmerged #390 extends Range/Pair/Dictionary evidence, and closed/unmerged #395 confirms recursive Pair projection across left/right/balanced nesting through depth 4 with Range/None members. Collection remains intentionally fail-closed because current typed Collections do not retain enough list-origin information for the observed Markdown-list-only case. Clean-room PRs #382/#385/#386 establish top-level/default/strict `.error` behavior. Closed/unmerged #392/#393 establish one enclosing Container-family / unordered-list / blockquote output context, and merged PR #398 adds independently sourced `.error` spelling for the rendered source echo. Closed/unmerged clean-room #401 extends exact v2.5.1/v2.6.0 evidence to top-level inline, ordered-list, Stacked row/column/grid, Landscape, and one deeper `.center` → `.row` composition; Arkst implements those bounded recovery/strict contexts while retaining exact 1:1 E3011 ↔ `ExplicitError` span/message plus evidenced source-echo pairing. Deeper-than-evidenced structural compositions, value contexts, orphan components, unrelated errors, and remaining logger String categories such as Collection remain #197 residuals; exact Quarkdown HTML/CSS styling is not a Typst/PDF closure requirement. Public WASM/embedder exposure remains #191-owned. |
