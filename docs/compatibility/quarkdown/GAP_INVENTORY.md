...
| Partially implemented (bounded evaluator) | `.extend`, `.super` | Source-defined callables and regular scalar builtins resolve through the existing evaluator and shared binder. Target parameter names, conditional delegation, chained parent links, scope layering, source-backed failures, and #167 savepoint rollback are independently tested in `crates/scribium-core/tests/quarkdown_function_extension.rs`. Specialized native owners, complete failure-effect parity, and rendered output remain outside this evaluator-only slice. |
