...
| Bounded semantic evidence (Unicode string semantics) | `.capitalize`, `.startswith` | `.capitalize` uses the pinned JDK 17-compatible Unicode 13.0 titlecase data, including multi-code-point mappings, and `.startswith(ignorecase:true)` uses Kotlin/JVM-compatible character-wise simple case comparison without normalization or whole-string case conversion. Independent `ǳ`/`ǲ`, `ß`, Greek `ς`/`Σ`, long-s, dotted-I, decomposed-prefix, and post-Unicode-13 `Ꟑ`/`ꟑ` cases are covered; #172 closes this bounded string-semantics slice at `SUPPORTED_SEMANTICS`. |
