...
| Intentionally deferred | Unimplemented data-loading families, `.llmstxt`, function-driven metadata, container/align layout families, arbitrary comparator syntax not present in v2.5.1, and generalized DynamicValue conversion |
...
| Layout and document functions | `.row`, `.column`, `.grid`, `.center`, `.container`, `.align`, `.box`, `.figure`, page breaks, tables, and related layout primitives. Evidence: [`Layout.kt`](https://raw.githubusercontent.com/iamgio/quarkdown/v2.5.1/quarkdown-stdlib/src/main/kotlin/com/quarkdown/stdlib/Layout.kt), [`Primitives.kt`](https://raw.githubusercontent.com/iamgio/quarkdown/v2.5.1/quarkdown-stdlib/src/main/kotlin/com/quarkdown/stdlib/Primitives.kt). | **Implemented for the reviewed block-body Stacked slice and bounded `.center` consumer.** `.row`, `.column`, and `.grid` bind typed arguments; `.center` accepts no non-body arguments. All validate before lazy body evaluation, preserve structured children, materialize as typed block nodes, and lower through the Typst backend. Direct `container`, `align` argument binding/conversion, `StyleOptions`, `float`, `fullspan`, and other layout primitives remain deferred. | `crates/scribium-core/tests/quarkdown_stacked_layout.rs`; `crates/scribium-core/tests/quarkdown_center.rs`; `crates/scribium-typst/tests/backend_integration.rs`; `examples/stacked/main.qd`; `examples/center/main.qd`. | Stacked layout family and bounded `.center` implemented for the reviewed block-body boundary; general String → Markdown conversion, inline Container insertion, `.align`, direct `.container`, `StyleOptions`, `.float`, and `.fullspan` remain deferred. |
...
## Stacked layout and center consumers
...
The bounded `.center` consumer adds only a full-width Container with logical
`Center` alignment, required Markdown block-body evaluation, typed nested
composition, and source-backed failure atomicity. It does not add `.align`
binding/conversion, direct `.container`, `StyleOptions`, `.float`, `.fullspan`,
general String → Markdown conversion, or inline Container insertion.

Deferred from this slice:

- general DynamicValue String → Markdown body conversion;
- inline Stacked insertion (Stacked is block-only); and
- `.align`, direct `.container`, and the remaining layout/component families.

## Selection record

The selected slices are the Stacked layout family (`.row`, `.column`, and
`.grid`) and the bounded `.center` consumer, integrated with the existing typed
...
deferred candidate is the direct `container` / `align` family after the remaining
#61 semantic gap inventory is re-reviewed. `.align` argument binding/conversion,
`StyleOptions`, `.float`, and `.fullspan` remain deferred.
