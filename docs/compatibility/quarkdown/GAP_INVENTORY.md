...
|---|---|---|---|---|
| Iteration | `Flow.kt` defines `.foreach` as an ordered map over any `Iterable` and `.repeat` as `.foreach {1..N}`. The v2.5.1 loops documentation also shows an inline lambda body and states that Markdown lists are accepted. Evidence: [`Flow.kt`](https://raw.githubusercontent.com/iamgio/quarkdown/v2.5.1/quarkdown-stdlib/src/main/kotlin/com/quarkdown/stdlib/Flow.kt), [loops](https://quarkdown.com/wiki/loops/), [`LoopTest.kt`](https://raw.githubusercontent.com/iamgio/quarkdown/v2.5.1/quarkdown-test/src/test/kotlin/com/quarkdown/test/LoopTest.kt). | Typed block and bounded positional inline `.foreach`/`.repeat` bodies, finite and left-open range materialization, Pair destructuring, ordered results, child isolation, failure atomicity, source-defined shadowing, and R10 materialization/depth budgets are implemented. Inline support is limited to the evidenced callable-body position; `.map`/`.filter` are Scribium extensions and `.sorted` covers the reviewed selector boundary. | Partially compatible; the documented iteration slice is covered, while broader upstream consumer coverage remains absent. | R12 completes the evidenced inline iteration-body slice through a source-backed contextual `Value::InlineBody`/`IrValue::InlineBody` carrier. It is adapted to the existing `IrCallable` path only after native target resolution; generalized inline component bodies and unrelated consumer families remain deferred. |
...
| Scoping/evaluation | v2.5.1 `Flow.kt` searches parent contexts for the existing variable owner before replacing a variable, evaluates the RHS in that owner context, and the pinned `VariableTest.kt` verifies that `.var {total} ...` inside `.foreach` updates the outer `total`. `Lambda.kt` forks invocation context and installs parameters last. Evidence: [`Flow.kt`](https://raw.githubusercontent.com/iamgio/quarkdown/v2.5.1/quarkdown-stdlib/src/main/kotlin/com/quarkdown/stdlib/Flow.kt), [`VariableTest.kt`](https://raw.githubusercontent.com/iamgio/quarkdown/v2.5.1/quarkdown-test/src/test/kotlin/com/quarkdown/test/VariableTest.kt). | Callable invocation records semantic writes, evaluates an existing-owner RHS without a same-name parameter shadow, publishes only names already visible in the caller after successful completion, and preserves new-local isolation. | Partially compatible; the reviewed owner/RHS slice is implemented while broader context/library behavior remains absent. | R11/R12 tests cover owner writeback, parameter shadowing, block/inline parity, and failure rollback. Definition-capture mutation and broader context/library sharing remain deferred. |
