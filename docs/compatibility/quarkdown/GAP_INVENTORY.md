...
[#195](https://github.com/luceat-lux-vestra/arkst/issues/195), while
`.log`/`.debug`/`.error` remain owned by
...
| Partially compatible | Function declarations/calls; variables; conditionals; lambdas/callables; iteration; collections; Dictionary/Pair/Range consumers outside the bounded `.get` lookup; type/value conversion; strings/text; bounded library inspection; metadata/document setup; bounded project data/file identity (`.filename`, `.listfiles`); complete public component/style/layout surface; complete error taxonomy |
| Compatible at the evidenced bounded boundary | Mathematics/numeric operations |
| Unsupported | Layout/document functions outside the reviewed Stacked, Container, and Landscape slices; remaining unimplemented #189 data-file families `.csv` and `.bibliography`; #151 logger family; #154 `.match`, `.loremipsum`, and `.subdocumentgraph` pending their explicit owner/blocker |
