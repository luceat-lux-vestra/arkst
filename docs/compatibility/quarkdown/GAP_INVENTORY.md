...
[#195](https://github.com/luceat-lux-vestra/arkst/issues/195). Bounded
logger/diagnostic semantics are also `PARTIAL` under
...
| Bounded semantic evidence (logger/diagnostics) | `.log`, `.debug`, `.error` | #197 implements shared binding, an explicit platform-neutral `LogSink`, deterministic no-sink behavior, source-backed ordered events, resource/library + sink composition, source-defined shadowing, and structured `.error` diagnostics. #368/PR #371 models the independently evidenced Unit result category. Clean-room PR #378 adds the initial logger-only String subset observed identically on exact Quarkdown v2.5.1/v2.6.0 artifacts; closed/unmerged clean-room PR #390 extends that stable subset to all Range endpoint shapes, the evidenced nested-Pair/None and Pair/Range compositions, and the evidenced flat ordered Dictionary scalar-entry JVM-style projection. Its observed Markdown-list Collection conversion remains intentionally unimplemented because current typed Collections do not retain enough list-origin information for a bounded adapter. Closed/unmerged #390 extends Range/Pair/Dictionary evidence, and closed/unmerged #395 confirms recursive Pair projection across left/right/balanced nesting through depth 4 with Range/None members. Clean-room PR #382 additionally pins default/non-strict `.error` rc=0, stable one-line stderr, rendered call-site continuation, source-defined-function behavior, and strict rc=66/no-artifact across the same exact releases; PR #385 further pins selected conditional/function body replacement plus caller continuation and ordered repeated top-level errors. Closed/unmerged clean-room PR #386 pins the strict host-finalization boundary: evaluation/log side effects retain ordinary semantics after the first top-level error, function/selected-conditional local bodies still stop at the error while caller/outer content continues, unselected conditionals remain lazy, the first selected error determines the bounded three-line stderr prefix, exit is 66, and no artifact is published. Arkst materializes a backend-neutral `ExplicitError` component, preserves failure-style rollback/value-boundary stopping, lets default native `arkst build` continue only when each dedicated explicit-error diagnostic is paired 1:1 with its materialized component, and applies `build --strict` only as a native host finalization policy after that same classification; unmaterialized/orphan explicit errors and every unrelated error remain fatal on the ordinary path. The generic scalar String adapter remains unchanged. Native `arkst build` supplies an explicit host-owned stdout sink for `Log` events while keeping `Debug` silent; ordinary core compilation remains no-sink/fail-closed for `.log`. Arkst-specific `check`/`inspect` additionally use an explicit sinkless/fail-closed analysis policy: no `LogSink`, `.log` remains E3010, `.debug` stays silent, and `.error` remains fatal E3011. `check` preserves all-diagnostics stderr output; `inspect` surfaces fatal diagnostics before refusing output while preserving warning-only stderr silence on successful inspection. The rows stay `PARTIAL`: exact Quarkdown HTML/CSS error-card styling and unevidenced output contexts plus remaining logger String categories such as Collection remain #197 residuals; public WASM/embedder exposure remains deferred under #191. #190's deterministic environment-input boundary is complete and is no longer a logger blocker. |
