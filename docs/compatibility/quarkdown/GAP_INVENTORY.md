...
| Historical #151 defer/handoff seed (not current status) | `.csv`, `.css`, `.cssproperties`, `.env`, `.filename`, `.htmloptions`, `.includeall`, `.listfiles`, `.llmstxt`, `.pathtoroot`, `.subdocument` | This row preserves the original family-level routing only. It is not a current per-surface classification: `.filename` and `.listfiles` are bounded `PARTIAL` project/resource rows under canonical #155/#189, `.env` is a bounded `PARTIAL` deterministic external-input row under #155/#190 with public WASM exposure left to #191, and the other names must be read from the current #151/#154/#155 manifests and active owners rather than inferred as uniformly deferred. |
