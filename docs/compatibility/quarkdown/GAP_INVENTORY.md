...
| Bounded semantic evidence (Unicode string semantics) | `.capitalize`, `.startswith` | `.capitalize` uses pinned Unicode titlecase data, including multi-code-point mappings, and `.startswith(ignorecase:true)` uses Kotlin/JVM-compatible character-wise simple case comparison without normalization or whole-string case conversion. Independent `ǳ`/`ǲ`, `ß`, Greek `ς`/`Σ`, long-s, dotted-I, and decomposed-prefix cases are covered; #172 closes this bounded string-semantics slice at `SUPPORTED_SEMANTICS`. |
