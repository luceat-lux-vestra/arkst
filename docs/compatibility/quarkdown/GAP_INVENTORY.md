...
| Bounded semantic evidence (logger/diagnostics) | `.log`, `.debug`, `.error` | #197 implements shared binding, an explicit platform-neutral `LogSink`, deterministic no-sink behavior, source-backed ordered events, resource/library + sink composition, source-defined shadowing, and structured `.error` diagnostics. #368/PR #371 models the independently evidenced Unit result category. Clean-room PR #378 adds the logger-only String subset observed identically on exact Quarkdown v2.5.1/v2.6.0 artifacts: Unit -> `kotlin.Unit`, None -> `None`, closed Range -> `start..end`, and the evidenced plain-text Pair JVM representation. Clean-room PR #382 additionally pins default/non-strict `.error` rc=0, stable one-line stderr, rendered call-site continuation, source-defined-function behavior, and strict rc=66/no-artifact across the same exact releases; PR #385 further pins selected conditional/function body replacement plus caller continuation and ordered repeated top-level errors; PR #386 pins strict evaluation/finalization ordering, first-error reporting, post-error top-level/caller logger observability, and unselected-conditional laziness. Arkst materializes a backend-neutral `ExplicitError` component, preserves failure-style rollback/value-boundary stopping, and lets default native `arkst build` continue only when each dedicated explicit-error diagnostic is paired 1:1 with its materialized component; strict native build preserves those evaluator semantics and converts the first paired explicit error into bounded strict stderr + exit 66 before artifact publication. Unmaterialized/orphan explicit errors and every unrelated error remain fatal. The generic scalar String adapter remains unchanged. Native `arkst build` supplies an explicit host-owned stdout sink for `Log` events while keeping `Debug` silent; ordinary core compilation remains no-sink/fail-closed for `.log`. The rows stay `PARTIAL`: exact Quarkdown HTML/CSS error-card styling and unevidenced output contexts, exact JVM strict stack traces, unreviewed logger String categories, and Arkst-specific analysis-command logger output policy remain #197 residuals; public WASM/embedder exposure remains deferred under #191. #190's deterministic environment-input boundary is complete and is no longer a logger blocker. |
