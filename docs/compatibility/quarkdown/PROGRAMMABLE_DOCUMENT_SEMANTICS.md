...
`.docdescription`, `.doctype`, and the bounded `.docauthor` slice support the
...
minimal ordered author records while reading the first author name. The
snapshot is serializable plain data with explicit defaults for older
serialized IR; the document type defaults to `plain` and missing authors
default to an empty collection. `.docauthors` and the remaining document
metadata fields remain deferred.
...
| Document-state mutation | Document APIs read with no argument, mutate shared mutable document info with an argument, and return void | Evaluator-owned state shared by ordinary callable child scopes and caller-overlay invocations; final `IrMetadata.document_state` snapshot; `.docname`, `.docdescription`, `.doctype`, and bounded `.docauthor` are implemented with bounded conversion | Evaluator-owned shared working state plus final `IrDocument` snapshot | Document State Foundation and caller sharing implemented; `.docname`, `.docdescription`, `.doctype`, and bounded `.docauthor` implemented | `.docauthors` and remaining document fields |
...
remain deferred. `.docauthors`, `.dockeywords`, `.doclang`, and `.theme` also
remain deferred; `.docauthor` is implemented only at its bounded name-only
