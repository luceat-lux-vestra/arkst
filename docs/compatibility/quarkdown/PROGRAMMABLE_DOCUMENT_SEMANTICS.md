...
`.docdescription`, `.doctype`, and the bounded `.docauthor`/`.docauthors`
slice support the
read/write dual API; writes return no document output, `.docname` rejects
blank values before mutation, `.doctype` validates a closed
`plain`/`paged`/`slides`/`docs` enum before mutation, and `.docauthor` appends
minimal ordered author records while reading the first author name; `.docauthors`
appends validated authors with ordered string info and returns an ordinary
typed dictionary view. The
snapshot is serializable plain data with explicit defaults for older
serialized IR; the document type defaults to `plain`, missing authors default
to an empty collection, and missing author info defaults to empty. The
...
| Document-state mutation | Document APIs read with no argument, mutate shared mutable document info with an argument, and return void | Evaluator-owned state shared by ordinary callable child scopes and caller-overlay invocations; final `IrMetadata.document_state` snapshot; `.docname`, `.docdescription`, `.doctype`, and bounded `.docauthor`/`.docauthors` are implemented with bounded conversion | Evaluator-owned shared working state plus final `IrDocument` snapshot | Document State Foundation and caller sharing implemented; `.docname`, `.docdescription`, `.doctype`, `.docauthor`, and `.docauthors` implemented at bounded evidenced boundaries | `.dockeywords`, `.doclang`, `.theme`, rendering/layout metadata, and remaining document fields |
...
remain deferred. `.dockeywords`, `.doclang`, `.theme`, rendering policy,
front-matter/document-state merging, and other document metadata remain
deferred. `.docauthor` and `.docauthors` are implemented only at their
