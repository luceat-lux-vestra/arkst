...
|---|---|
| `PRODUCTION_GAP` | #165–#167, #169, #173–#185, #189, and #197–#199 where the required parser, engine, content, layout, or resource behavior is absent or only bounded. The bounded #195 inspection contract and completed #188 resolver are evidence rather than current owners; #197 remains open because its implemented logger dispatch still lacks upstream `kotlin.Unit` return semantics and richer DynamicValue-to-String parity in addition to host/output gaps. |
...
| [#197](https://github.com/luceat-lux-vestra/arkst/issues/197) | #151 / logger and diagnostic builtins | Implemented bounded evaluator-native `.log`, `.debug`, and `.error`: shared binding/scalar String conversion, explicit platform-neutral `LogSink`, deterministic no-sink `.log` rejection, silent no-sink `.debug`, source-backed ordered events, structured `.error` failure diagnostics, resource/library + sink composition, and source-defined shadowing. Clean-room v2.6 evidence is recorded in [`LOGGER_DIAGNOSTICS.md`](LOGGER_DIAGNOSTICS.md). The rows remain `PARTIAL`: upstream value-context `.log`/`.debug` results materialize as `kotlin.Unit`, richer DynamicValue String formatting is broader than Arkst's scalar boundary, and normal CLI stdout/error-card/strict-mode plus public WASM/embedder parity are not claimed. | Bounded evaluator/sink slice implemented, but #197 remains open for the owned return/value residuals; host/CLI/public-binding portions remain coordinated with #190/#191 and the relevant output boundary. |
