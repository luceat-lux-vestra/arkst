...
|---|---|
| `PRODUCTION_GAP` | #165–#167, #169, #173–#185, #189, and #197–#199 where the required parser, engine, content, layout, or resource behavior is absent or only bounded. The bounded #195 inspection contract and completed #188 resolver are evidence rather than current owners; #368/PR #371 corrects the #149 Unit value-model omission, while #197 now has the clean-room-evidenced Unit/None/closed-Range/plain-text-Pair logger String subset, native `arkst build` stdout wiring, and bounded default/non-strict `.error` stderr/rendered continuation, and remains open for unreviewed logger String categories plus strict-mode and exact-styling/unevidenced-output residuals; public WASM/embedder exposure remains #191. |
...
| `PLATFORM_GAP` | #190's deterministic host/process input boundary is completed; native `arkst build` logger stdout is now wired through an explicit host sink; remaining public WASM/provider exposure is #191 and remaining `.error` strict-mode and exact-styling/unevidenced-output behavior stays #197-owned. |
...
```text
  ...
  #165/#166/#167 ──► #194 dictionary, #196 localization
       ...
       #197 logger: bounded evaluator/sink + Unit + evidenced String subset + native build stdout + default/non-strict `.error` output implemented; strict-mode/styling/unevidenced-output and unreviewed String residuals stay #197, public WASM/embedder exposure stays #191 (#190 complete)
