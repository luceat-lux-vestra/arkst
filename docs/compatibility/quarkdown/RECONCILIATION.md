...
| #153 layout/configuration | [`LAYOUT_DOCUMENT_CONFIGURATION_AUDIT.md`](LAYOUT_DOCUMENT_CONFIGURATION_AUDIT.md) and [`LAYOUT_DOCUMENT_CONFIGURATION_AUDIT_MANIFEST.tsv`](LAYOUT_DOCUMENT_CONFIGURATION_AUDIT_MANIFEST.tsv) | 47 | 13 #153-owned `PARSED_ONLY`, 7 #153-owned `PARTIAL`; 27 handoffs | Bounded state/output slices are recorded conservatively; broader document-wide configuration is not promoted from parser retention or adjacent component behavior. |
...
| Remaining document-wide layout/configuration (`.numbering`, `.pageformat`, `.font`, page counters, navigation, `.slides`, and related rows) | #153; 9 bounded `PARTIAL` rows and 11 `PARSED_ONLY` rows | #154 component/content consumers; #175–#178 | Numbering/caption/paragraph-style, bounded size-only font state, and selected page/slides state have bounded implementations; all residual fields and downstream consumers retain explicit owners and conservative status. |
