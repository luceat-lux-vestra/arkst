...
```text
  ...
  #165/#166/#167 ──► #194 dictionary, #196 localization
       ...
       #197 logger: bounded evaluator/sink + Unit + evidenced String subset + native build stdout + default/non-strict `.error` output + strict host finalization + sinkless/fail-closed check/inspect policy implemented; merged #398 covers bounded source echo; #401/#431/#433 establish the supported structural/inline rendered-owner families and representative composition, while exact v2.5.1/v2.6.0 source review shows that upstream recursively composes nestable rendered owners without a depth counter; Arkst therefore keeps that supported output-owner tree recursively composable at arbitrary depth while retaining semantic-boundary and exact pairing gates; image-alt remains #182-dependent, operation-generated Collection projections remain explicitly fail-closed/non-portable, and public WASM/embedder exposure stays #191 (#190 complete)
