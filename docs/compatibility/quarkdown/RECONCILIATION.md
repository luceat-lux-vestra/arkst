...
| #151 stdlib/general builtins | [`STDLIB_BUILTINS_AUDIT.md`](STDLIB_BUILTINS_AUDIT.md) and [`STDLIB_BUILTINS_AUDIT_MANIFEST.tsv`](STDLIB_BUILTINS_AUDIT_MANIFEST.tsv) | 162 | 46 `SUPPORTED_SEMANTICS`, 13 `PARTIAL`, 0 `UNSUPPORTED`, 1 `NOT_APPLICABLE` among 60 #151-owned rows; 102 explicit handoffs | The complete pinned declaration sweep is retained; cross-owned names are not silently omitted or reclassified. |
...
| General stdlib declaration set and bounded scalar/numeric/collection functions | #151; exact per-name status in the 162-row manifest | #149 value boundary; #150 callback flow; #152–#155 consumers | The 162-name pinned sweep is complete. `.capitalize`/`.startswith` are `SUPPORTED_SEMANTICS` at the bounded scalar boundary after #172, `.get` is `SUPPORTED_SEMANTICS` through the #194 evaluator-native typed lookup, and `.localization`/`.localize` are `SUPPORTED_SEMANTICS` through the #196 evaluator-native table/lookup slice. The four #195 library-inspection rows are `PARTIAL`: registry semantics are implemented, but stdlib enumeration/existence remains support-filtered until the wider callable surface exists. The three #197 logger/diagnostic rows are also `PARTIAL`: evaluator dispatch/conversion, explicit sink/no-sink policy, provenance, native `arkst build` `Log` stdout, the clean-room-evidenced default/non-strict `.error` stderr + rendered-component continuation contract, and the closed/unmerged #386-evidenced native `build --strict` host-finalization boundary are implemented. Strict preserves ordinary evaluator/log continuation, reports the first paired explicit error with exit 66 and the bounded three-line stderr prefix, and publishes no artifact. Arkst-specific `check`/`inspect` are explicitly sinkless/fail-closed: `.log` is E3010, `.debug` is silent, and `.error` is fatal E3011. `check` preserves all-diagnostics stderr output; `inspect` surfaces fatal diagnostics before refusing output while preserving warning-only stderr silence on successful inspection. Merged #398 supplies bounded independently sourced `.error` source-echo output; closed/unmerged #401 additionally evidences top-level inline, ordered-list, Stacked row/column/grid, Landscape, and a representative `.center` → `.row` composition, while closed/unmerged #431 adds direct heading/table-cell and emphasis/strong/strikethrough/link-text ownership and closed/unmerged #433 establishes representative composability across the inline owner family and list/blockquote/table structural paths. Exact source review of Quarkdown v2.5.1/v2.6.0 confirms the same owner model in both tags: `FunctionCallNode` is `ErrorCapableNode`, an expansion failure is rendered as `Box.error`, and nestable rendered owners recurse through children without a depth counter. Arkst therefore does not classify arbitrary deeper permutations of already-supported rendered owners as separate gaps; those owners compose recursively while exact diagnostic/carrier/source-echo gates remain fail-closed. Closure review finds no additional #197-owned public error owner. Image-alt behavior observed by #431 remains #182-owned because the image/media path currently fails earlier with E8001; upstream `Numbered` uses a distinct late-evaluation error path but Arkst `.numbered` is #181-owned; closed/unmerged #403 narrows Collection behavior and #405 proves the direct-list projection preserves source spelling, while operation-generated Collection projections remain fail-closed because their observed JVM forms may contain unstable runtime context identities; public WASM/embedder exposure remains #191-owned. #197 implementation ownership is complete, while the three canonical rows remain `PARTIAL` for those cross-owned or intentionally non-portable boundaries; exact Quarkdown HTML/CSS styling is not a Typst/PDF closure requirement. |
...
| `.numbering`, `.nonumbering` | #153; both `PARTIAL` | #175 plus numbering-aware content/output consumers | Bounded binder/evaluator state, typed serializable numbering IR, merge/replace, explicit disable, token parsing, duplicate builtin storage in `extra`, and rollback are implemented. Concrete document-type default formats and all numbering-aware rendered consumers remain open, so no end-to-end claim is made. |
| Remaining document-wide layout/configuration (`.pageformat`, `.font`, page counters, navigation, `.slides`, and related rows) | #153; 17 `PARSED_ONLY` rows | #154 component/content consumers; #175–#178 | Parser/retention evidence alone does not establish state, IR, or output support. Existing bounded v2.6/pageformat/slides slices remain narrower than completion of these baseline rows; follow-up ownership remains grouped by contract. |
...
|---|---|
| `PRODUCTION_GAP` | #165–#167, #169, #173–#185, #189, and #198–#199 where the required parser, engine, content, layout, or resource behavior is absent or only bounded. The bounded #195 inspection contract and completed #188 resolver are evidence rather than current owners; #368/PR #371 corrects the #149 Unit value-model omission, while #197 now has the clean-room-evidenced Unit/None/all-Range/evidenced-recursive-Pair/flat-ordered-Dictionary-scalar-entry logger String subset, native `arkst build` stdout wiring, bounded default/non-strict `.error` stderr/rendered continuation, and bounded strict host finalization with exit 66/no-artifact/first-error stderr, and is complete as the logger/diagnostic implementation owner; row-level `PARTIAL` status is cross-owned (#181/#182/#191) or intentionally non-portable rather than #197 production work; #403's operation-generated Collection projections are classified as an explicitly fail-closed, non-portable JVM boundary after implementing the #405-evidenced source-spelling-preserving direct Markdown-list origin, not as additional production work. Public WASM/embedder exposure remains #191. |
...
| `PLATFORM_GAP` | #190's deterministic host/process input boundary is completed; native `arkst build` logger stdout is now wired through an explicit host sink; remaining public WASM/provider exposure is #191; `.error` strict native-build finalization and Arkst-specific sinkless/fail-closed analysis-command policy are bounded under #197, while merged #398 owns the bounded source-echo slice and #197 implementation ownership is complete; remaining numbered/image/embedder boundaries are #181/#182/#191-owned rather than depth combinations within the supported rendered-owner tree; exact Quarkdown HTML/CSS styling is not a Typst/PDF closure requirement. |
...
| [#195](https://github.com/luceat-lux-vestra/arkst/issues/195) | #151 / library/runtime inspection | Implemented bounded evaluator-native `.libexists`, `.functionexists`, `.libraries`, and `.libfunctions` over one deterministic ordered in-memory registry, including source-function pseudo-libraries, exact unknown behavior, source-backed diagnostics, and transaction rollback. It performs no provider/filesystem/process/network/environment discovery. The four canonical rows remain `PARTIAL` because stdlib visibility is deliberately limited to Arkst-callable functions instead of pretending unsupported upstream functions exist. | Completed bounded runtime-inspection slice; broader stdlib callable parity is owned by the corresponding function families, not by a second registry or host capability. |
...
| [#197](https://github.com/luceat-lux-vestra/arkst/issues/197) | #151 / logger and diagnostic builtins | Implemented bounded evaluator-native `.log`, `.debug`, and `.error`: shared binding, explicit platform-neutral `LogSink`, deterministic no-sink `.log` rejection, silent no-sink `.debug`, source-backed ordered events, structured `.error` failure diagnostics, resource/library + sink composition, and source-defined shadowing. #368/PR #371 models the independently evidenced Unit result semantics. Clean-room PR #378 establishes the initial logger-only `message: String` subset; closed/unmerged #390 extends the identical cross-version evidence to all Range endpoint shapes, initial nested Pair compositions, and the evidenced flat ordered Dictionary scalar-entry JVM-style projection; closed/unmerged #395 independently confirms recursive Pair projection across left/right/balanced nesting through depth 4 with Range/None members without widening unrelated generic String consumers. Closed/unmerged #403 proves Collection origin is semantically relevant and #405 proves the direct-list projection is source-spelling-sensitive; Arkst implements that bounded direct Markdown-list origin with evaluator-local raw-body text consumed only by a same-scope direct variable reference, while operation-generated Collections remain fail-closed because their observed JVM projections differ and may contain unstable runtime context identities. The rows remain `PARTIAL`: native `arkst build` `Log` stdout, clean-room-evidenced default/non-strict `.error` stderr/rendered continuation, and closed/unmerged #386-evidenced strict host finalization are implemented; strict retains ordinary evaluator/log continuation, reports the first paired explicit error with exit 66 and the bounded three-line stderr prefix, and suppresses artifact/backend publication. Arkst-specific `check`/`inspect` now have the explicit sinkless/fail-closed policy described above. Operation-generated Collection projections are explicitly fail-closed as a non-portable JVM boundary rather than an additional production target; merged #398 covers the bounded source-echo slice; closed/unmerged #401 evidences top-level inline, ordered-list, Stacked row/column/grid, Landscape, and a representative `.center` → `.row` composition; closed/unmerged #431 additionally evidences direct heading/table-cell and emphasis/strong/strikethrough/link-text owners; closed/unmerged #433 establishes representative nested composition across unordered-list→emphasis, ordered-list→link, blockquote→strong, emphasis→link, link→strong, and table-cell→strong. Exact v2.5.1/v2.6.0 source review confirms that these rendered owners are recursively composed by the upstream AST/error path without a nesting-depth counter, so the production slice preserves arbitrary depth across the already-supported rendered-owner tree rather than adding per-depth gates. Recovery remains fail-closed through exact E3011/carrier/source-echo pairing, and distinct semantic boundaries are not generalized. #431's distinct image-alt observation remains a #182-dependent residual because native Markdown image output still fails earlier with E8001; exact Quarkdown HTML/CSS styling is not a Typst/PDF closure requirement; public WASM/embedder parity is #191-owned. | Bounded evaluator/sink + Unit + evidenced logger String subset + native `arkst build` stdout + default/non-strict `.error` output + native strict finalization + Arkst analysis-command logger policy implemented. #190 is complete; #197 implementation ownership is complete. Image-alt remains #182-owned, upstream `Numbered`'s distinct late-evaluation path maps to #181, and #191 remains the deferred public WASM/embedder owner; operation-generated Collection projections remain intentionally non-portable rather than a #197 production target. The canonical logger/diagnostic rows therefore remain `PARTIAL` without keeping #197 active. |
...
| [#189](https://github.com/luceat-lux-vestra/arkst/issues/189) | #155 / project data and file identity | `.listfiles`, `.filename`, bounded raw `.csv` resource parsing, `.bibliography`; depends on completed #188 and coordinates with #181/#183. | Active resource/data owner after completed #188; `.csv` parser/resource is bounded `PARTIAL` while public callable/table production remains #183-owned; exact per-row status remains in the filesystem manifest. |
...
```text
  ...
  #165/#166/#167 ──► #194 dictionary, #196 localization
       ...
       #197 logger (completed owner): bounded evaluator/sink + Unit + evidenced String subset + native build stdout + default/non-strict `.error` output + strict host finalization + sinkless/fail-closed check/inspect policy implemented; merged #398 covers bounded source echo; #401/#431/#433 establish the supported structural/inline rendered-owner families and representative composition, while exact v2.5.1/v2.6.0 source review shows that upstream recursively composes nestable rendered owners without a depth counter; Arkst therefore keeps that supported output-owner tree recursively composable at arbitrary depth while retaining semantic-boundary and exact pairing gates. Remaining row-level limits are cross-owned by #181 `Numbered`, #182 image/media, and #191 public WASM/embedder, plus the intentionally non-portable operation-generated Collection boundary (#190 complete)
...
- every actionable partial/unsupported/unknown resource and implementation gap
  points to the listed bounded follow-ups (including #198–#199), an explicit
