...
|---|---|---:|---|---|
| #148 grammar/frontend | [`CALL_GRAMMAR_AUDIT.md`](CALL_GRAMMAR_AUDIT.md); `crates/scribium-markdown/tests/call_grammar_audit.rs` | 15 | 9 `PARTIAL`, 6 `PARSED_ONLY` | Recognition/provenance only; #149 owns binding and #150 owns evaluation. #157 and #163's bounded grammar/frontend slices are implemented; remaining grammar gaps retain their row statuses. |
...
| [#163](https://github.com/luceat-lux-vestra/scribium/issues/163) | #148 → #149 / grammar representation with engine handoff | Implemented in the grammar/frontend: head and chain segments preserve one source-ordered argument sequence with provenance. A transitional engine handoff guard rejects positional-after-named with source-backed `E3003` before the legacy split IR projection; #165 must absorb it into the shared binder. | Completed in this bounded frontend/engine-safety band; complete semantic binding remains #165. |
