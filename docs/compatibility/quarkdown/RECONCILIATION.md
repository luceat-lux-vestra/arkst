...
|---|---|
| `PRODUCTION_GAP` | #165–#167, #169, #173–#185, #189, and #197–#199 where the required parser, engine, content, layout, or resource behavior is absent or only bounded. The bounded #195 inspection contract and completed #188 resolver are evidence rather than current owners; #368/PR #371 corrects the #149 Unit value-model omission, while #197 now has the clean-room-evidenced Unit/None/closed-Range/plain-text-Pair logger String subset and remains open for unreviewed logger String categories plus rendered-error/stderr/strict residuals; native CLI build `.log` stdout integration is complete through the explicit sink boundary, and public WASM/embedder exposure remains #191. |
...
| `PLATFORM_GAP` | #190's deterministic host/process input boundary is completed; remaining public WASM/provider exposure is #191, while native logger CLI sink/output integration remains #197-owned. |
...
```text
  ...
  #165/#166/#167 ──► #194 dictionary, #196 localization
       ...
       #197 logger: bounded evaluator/sink + Unit + evidenced logger String subset implemented; native CLI/output/strict and unreviewed String residuals stay #197, public WASM/embedder exposure stays #191 (#190 complete)
