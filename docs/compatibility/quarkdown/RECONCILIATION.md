...
|---|---|
| `PRODUCTION_GAP` | #165–#167, #169, #173–#185, #189, and #197–#199 where the required parser, engine, content, layout, or resource behavior is absent or only bounded. The bounded #195 inspection contract and completed #188 resolver are evidence rather than current owners; #368/PR #371 corrects the #149 Unit value-model omission, while #197 now has the clean-room-evidenced Unit/None/closed-Range/plain-text-Pair logger String subset, native `arkst build` stdout wiring, bounded default/non-strict `.error` stderr/rendered continuation, and bounded native strict exit-66/no-artifact finalization, and remains open for unreviewed logger String categories plus exact-styling/unevidenced-output, exact strict-stack, and analysis-command residuals; public WASM/embedder exposure remains #191. |
...
| [#197](https://github.com/luceat-lux-vestra/arkst/issues/197) | #151 / logger and diagnostic builtins | Implemented bounded evaluator-native `.log`, `.debug`, and `.error`: shared binding, explicit platform-neutral `LogSink`, deterministic no-sink `.log` rejection, silent no-sink `.debug`, source-backed ordered events, structured `.error` failure diagnostics, resource/library + sink composition, and source-defined shadowing. #368/PR #371 models the independently evidenced Unit result semantics. Clean-room PR #378 extends the logger-only `message: String` boundary identically across exact v2.5.1/v2.6.0 artifacts for Unit (`kotlin.Unit`), None (`None`), closed Range (`1..3`), and the evidenced plain-text Pair JVM representation without widening unrelated generic String consumers. The rows remain `PARTIAL`: native `arkst build` `Log` stdout plus clean-room-evidenced default/non-strict and bounded native strict `.error` behavior are implemented, including first-error strict stderr, exit 66/no-artifact finalization, caller/top-level continuation, and unselected-conditional laziness; unreviewed logger String categories, exact HTML/CSS styling/unevidenced output contexts, exact JVM strict stack traces, and Arkst-specific analysis-command logger output policy remain outside the bounded slice; public WASM/embedder parity is #191-owned. | Bounded evaluator/sink + Unit + evidenced logger String subset + native `arkst build` stdout + default/non-strict + strict `.error` native-build behavior implemented. #190 is complete; #197 remains open for styling/unevidenced-output, strict-stack, analysis-command, and unreviewed String-category residuals, while #191 remains the deferred public WASM/embedder owner. |
...
```text
  ...
  #165/#166/#167 ──► #194 dictionary, #196 localization
       ...
       #197 logger: bounded evaluator/sink + Unit + evidenced String subset + native build stdout + default/non-strict + native strict `.error` behavior implemented; styling/unevidenced-output, exact strict-stack, analysis-command, and unreviewed String residuals stay #197, public WASM/embedder exposure stays #191 (#190 complete)
