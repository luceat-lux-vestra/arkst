...
| `PLATFORM_GAP` | #190's deterministic host/process input boundary is completed; native `arkst build` logger stdout is now wired through an explicit host sink; remaining public WASM/provider exposure is #191 and remaining `.error` native output/strict behavior stays #197-owned. |
...
```text
  ...
  #165/#166/#167 ──► #194 dictionary, #196 localization
       ...
       #197 logger: bounded evaluator/sink + Unit + evidenced String subset + native build stdout implemented; `.error` output/strict and unreviewed String residuals stay #197, public WASM/embedder exposure stays #191 (#190 complete)
