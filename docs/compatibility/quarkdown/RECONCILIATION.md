...
|---|---|---:|---|---|
| #148 grammar/frontend | [`CALL_GRAMMAR_AUDIT.md`](CALL_GRAMMAR_AUDIT.md); `crates/arkst-markdown/tests/call_grammar_audit.rs` | 15 | 6 `PARTIAL`, 9 `PARSED_ONLY` | Recognition/provenance only; #165 owns shared structural binding, #149 owns target-driven conversion, and #150 owns evaluation. #157, #158, #163, and #164's bounded grammar/frontend slices are implemented; remaining grammar, protected-context, and broader content rows retain their conservative statuses. |
...
| [#164](https://github.com/luceat-lux-vestra/arkst/issues/164) | #148 / Markdown and Quarkdown frontend | Implemented bounded separator scanning before the first and subsequent arguments and before `::`, plus valid trailing-continuation consumption without a fabricated argument. `arkst-markdown` keeps pending block headers aligned with the grammar at physical-line boundaries. Exact source spans, block/inline placement, LF evidence, and separate actual-CRLF local evidence are covered; binder/evaluator behavior is out of scope. | Completed bounded parser/frontend slice; broader grammar and semantic/output contracts remain separate. |
