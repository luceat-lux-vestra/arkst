...
- [ ] `slides` PDF fixes that are observable in Arkst-owned export behavior — implementation candidate #358 now contains the bounded #175/#178/#185 subcontracts and artifact-level evidence documented in `V260_SLIDES_PDF_EXPORT.md`; this row remains unchecked until fresh production CI verifies subprocess/in-process parity, WASM, and the cross-platform required contexts on the final candidate HEAD.
