...
[#159], [#160], [#162], and [#164], all native sub-issues of #148:
...
| [#163](https://github.com/luceat-lux-vestra/scribium/issues/163) | Implemented: the grammar and frontend retain one source-ordered argument sequence, including positional-after-named transitions, with source-backed spans. A transitional engine handoff guard rejects that invalid ordering with source-backed `E3003` before the legacy split IR projection. | `scribium-quarkdown` / `scribium-markdown` representation plus bounded `scribium-engine` handoff; #165 shared binder replacement |
...
`E2001` for positional-after-named. Before the current IR receives its existing
derived positional/named projections, the engine handoff guard rejects the
invalid shape with `E3003`, using the positional `CallArgument` span as the
primary and the preceding named argument span as a secondary location; the
invalid call or chain is omitted from IR. This is a bounded safety guard, not
the complete binder: shared parameter metadata and all other semantic binding
rules remain #165/#149 work, and no output compatibility claim is made.
...
| Named and mixed argument shape | Public syntax page; pinned `FunctionCallGrammar.kt` optional-name sequence, shared identifier pattern, and adjacent `identifier ":" "{"` delimiter grammar, plus `RegularArgumentsBinder.kt` binder rule | `.foo named:{x} {y}`; `.foo name :{x}`; `.foo name: {x}`; `.foo name : {x}` | Scribium shares the pinned identifier scanner and requires adjacent `identifier:{` for named arguments. Nonmatching whitespace/invalid-identifier forms remain source remainder without a fabricated `E2002`; matched named values and all mixed transitions are retained in one source-ordered frontend `CallArgument` sequence. Positional-after-named is no longer rejected by the parser; the AST-to-IR handoff guard rejects it with source-backed `E3003` before the legacy split IR projection | Named name/value/whole-argument spans, positional argument spans, and nonmatching boundary spans are source-backed; the handoff diagnostic uses the actual positional and named argument spans, with no source reconstruction | Frontend mode-isolation and `.qd`/`.md` probes pass for block, inline, and chained calls; semantic guard coverage includes head and chain paths | LF and UTF-8/real-CRLF name/value/order span probes pass locally; no upstream raw-CRLF claim is made for the separate continuation token | The three whitespace variants remain source remainder without diagnostics; a matched `name:{` with an unclosed value remains `E2003`; positional-after-named reaches the frontend without parser `E2001` and is rejected at engine handoff with `E3003` | `crates/scribium-markdown/tests/call_grammar_audit.rs::audit_preserves_ordered_mixed_arguments_until_binder_validation`; `crates/scribium-core/src/lib.rs::compile_rejects_positional_after_named_at_engine_handoff`; named-identifier and malformed argument tests | Grammar/frontend shape plus one bounded engine safety guard; no complete binder conformance claim | `PARSED_ONLY` | #157 implemented; #163 implements the representation and transitional handoff guard; #149/#165 own the shared binder replacement; #60/#65 | Keep lexical adjacency and source-order/provenance here; #165 must absorb the guard into shared validation with all other binding rules | `scribium-quarkdown`; `scribium-markdown`; bounded `scribium-engine` handoff; #149/#165 |
...
- `docs/SYNTAX.md` now records the pinned call-identifier, numeric-reference,
  ...
  contract, while retaining separate notes for the remaining #158–#160, #162,
  and #164 gaps.
