...
| [#163](https://github.com/luceat-lux-vestra/scribium/issues/163) | Implemented: the grammar and frontend retain one source-ordered argument sequence, including positional-after-named transitions, with source-backed spans. The semantic validity check remains binder-owned. | `scribium-quarkdown` / `scribium-markdown` representation; #165 engine handoff |
...
`E2001` for positional-after-named. The current IR continues to receive its
existing derived positional/named projections at the AST-to-IR boundary; this
change does not implement binder validation, evaluator behavior, or output
compatibility. General semantic rejection and binder metadata remain #165/#149
work.
...
| Named and mixed argument shape | Public syntax page; pinned `FunctionCallGrammar.kt` optional-name sequence, shared identifier pattern, and adjacent `identifier ":" "{"` delimiter grammar, plus `RegularArgumentsBinder.kt` binder rule | `.foo named:{x} {y}`; `.foo name :{x}`; `.foo name: {x}`; `.foo name : {x}` | Scribium shares the pinned identifier scanner and requires adjacent `identifier:{` for named arguments. Nonmatching whitespace/invalid-identifier forms remain source remainder without a fabricated `E2002`; matched named values and all mixed transitions are retained in one source-ordered frontend `CallArgument` sequence. Positional-after-named is no longer rejected by the parser; semantic validity remains binder-owned | Named name/value/whole-argument spans, positional argument spans, and nonmatching boundary spans are source-backed; no source is reconstructed | Frontend mode-isolation and `.qd`/`.md` probes pass for block, inline, and chained calls | LF and UTF-8/real-CRLF name/value/order span probes pass locally; no upstream raw-CRLF claim is made for the separate continuation token | The three whitespace variants remain source remainder without diagnostics; a matched `name:{` with an unclosed value remains `E2003`; positional-after-named reaches the frontend representation without parser `E2001` | `crates/scribium-markdown/tests/call_grammar_audit.rs::audit_preserves_ordered_mixed_arguments_until_binder_validation`, `audit_aligns_named_argument_identifier_lexing_and_spans`, `audit_requires_adjacent_named_argument_delimiters_and_preserves_source`; existing malformed argument tests | Independent grammar/frontend shape evidence only; no binder or semantic conformance claim | `PARSED_ONLY` | #157 implemented; #163 implemented representation boundary; #149/#165 binder/value ownership; #60/#65 | Keep lexical adjacency and source-order/provenance here; leave positional-after-named validity and all other binding rules to #149/#165 | `scribium-quarkdown`; `scribium-markdown` representation; #149/#165 semantic owner |
