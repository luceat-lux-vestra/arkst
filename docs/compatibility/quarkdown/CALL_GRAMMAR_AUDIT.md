...
[#160], and [#162] are now implemented at the grammar/frontend boundary
below; the remaining bounded issue is [#164], a native sub-issue of #148:
...
| Inline vs block placement | Public syntax page; pinned `FunctionCallPatterns.kt` block/inline rule | `Text .foo {x} text` vs `.foo {x}` | Block parser accepts isolated calls; inline parser preserves calls inside paragraphs and trailing same-line text selects inline path | Call/paragraph spans remain source-backed | `.md` has no directive extension; `.qd` body/list/blockquote paths tested | LF/CRLF covered | Malformed inline recovery retains the consumed original segment and following source (#159) | `qd_mode_parses_root_and_inline_calls_with_crlf_provenance`, `malformed_inline_call_preserves_full_source_offset`, `malformed_inline_recovery_preserves_utf8_crlf_and_following_source`, body/container tests | Existing parser evidence only | `PARSED_ONLY` | #60/#65; #159/#162 implemented | Keep placement heuristic independent of evaluator; separator placement remains #164 | `arkst-markdown` |
...
| Source provenance | Accepted call, argument, named-name, segment, body, UTF-8, CRLF, escaped-delimiter, nested tight wrapper/inner spans, and malformed inline recovery are verified at the frontend boundary; separator placement remains the pinned-equivalent gap. Mixed argument order is retained at the grammar/frontend boundary, and the temporary engine guard reports from the actual argument spans before split IR projection; complete binder validation is not claimed. |
...
- `docs/SYNTAX.md` now records the pinned call-identifier, numeric-reference,
  ...
  contract, while retaining a separate note for the remaining #164 gap. The
  #159 recovery, #160 content-inline, and #162 escaped-delimiter frontend
  slices are recorded
  in the matrix.
...
- The remaining production gap is tracked as #164; #159's bounded
  parser/frontend recovery, #160's bounded content-inline slice, and #162's
  escaped-delimiter correction are implemented, while broader content
