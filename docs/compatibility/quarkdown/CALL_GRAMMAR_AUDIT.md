...
- after-argument backslash continuation accepts arbitrary leading spaces/tabs
  and LF/CRLF in the revalidated #60/#65 slice; #164 now aligns the same
  separator placement before the first argument, before `::`, and at the
  trailing edge;
...
| [#164](https://github.com/luceat-lux-vestra/arkst/issues/164) | Implemented bounded separator placement: optional horizontal whitespace and LF/CRLF continuation are consumed before every candidate argument and before `::`; a valid trailing continuation is consumed without fabricating an argument. | `arkst-quarkdown` grammar; `arkst-markdown` block/inline integration |
...
the first argument, between arguments, and before each chain `::`. The separate
trailing-continuation case accepts a final continuation and optional indentation
without requiring or fabricating an argument. Chain scanning retains the
separator only as part of the complete call range: head and chain segment spans
remain their own source-backed ranges.
...
| Line continuation and argument-separator placement | Public syntax page; pinned `FunctionCallGrammar.kt` `argumentSeparator` before every inline argument and `trailingLineContinuation`; pinned `FunctionCallTest.kt` continuation-before-first-argument/body and trailing-range evidence | `.foo \\\nname:{x}`; `.foo {x} \\\n` | `parse_arguments` consumes the shared private separator before the first and subsequent arguments; `parse_directive_at` reuses it before each `::`; a final continuation is accepted without an argument | Complete calls, arguments, named names/values, and chain ranges point into the unchanged source; the trailing continuation is included in the complete call range while the argument span is not widened | Positive block/inline Quarkdown probes pass; Markdown mode keeps the same source literal and isolated | The pinned continuation evidence is LF-specific. Actual CRLF cases pass as Arkst source-preservation/frontend evidence and are not raw-CRLF upstream conformance claims | Genuine malformed continuation/chain forms retain source-backed `E2004`; unclosed arguments retain `E2003` | `parses_argument_separators_before_first_argument_and_at_trailing_edge`, `parses_separator_before_chain_without_changing_segment_spans`, `trailing_continuation_accepts_optional_indentation_without_extra_argument`, `audit_accepts_continuation_before_first_argument_and_preserves_spans`, `audit_accepts_trailing_continuation_without_fabricating_an_argument`, `audit_accepts_whitespace_and_continuation_before_chain_separators` | Independent parser/frontend evidence for the bounded corrected placement; no evaluator/output claim | `PARSED_ONLY` | #164 implemented; #60/#65 | Keep binding, evaluation, chain value flow, and output compatibility separate | `arkst-quarkdown`; `arkst-markdown` integration |
