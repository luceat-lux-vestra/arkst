...
documentation overclaim. The lexical gap in [#157] is now implemented at the
grammar/frontend boundary below; the remaining bounded issues are [#158],
...
boundary row; the tests named below remain parser/provenance evidence only.
...
numeric identifiers include `0` and leading zeros and reject an immediately
adjacent ASCII alphanumeric suffix such as `.1abc`. The same scanner is used
for normal call segments and named-argument candidates.
...
forms remain outside `named_args`; the existing structured `E2002` diagnostic
is retained for an immediately recognized `name:` without an adjacent braced
value. Numeric identifiers remain grammar identifiers, so `.1 {item}` follows
the same argument-shape path; implicit-reference binding remains #150-owned.
...
| Named and mixed argument shape | Public syntax page; pinned `FunctionCallGrammar.kt` optional-name sequence, shared identifier pattern, and adjacent `identifier ":" "{"` delimiter grammar, plus `RegularArgumentsBinder.kt` binder rule | `.foo named:{x} {y}`; `.foo name :{x}`; `.foo name: {x}`; `.foo name : {x}` | Scribium now shares the pinned identifier scanner and requires adjacent `identifier:{` for named arguments. Nonmatching whitespace/invalid-identifier forms are not inserted into `named_args`; positional-after-named remains the existing grammar-layer `E2001` and is #163-owned for representation/binder alignment | Named name/value/whole-argument spans and nonmatching boundary spans are source-backed; the `E2001` opening-brace span remains source-backed | Frontend mode-isolation and `.qd`/`.md` probes pass | LF and UTF-8/CRLF name/value span probes pass locally; no upstream raw-CRLF claim is made for the separate continuation token | Missing adjacent braced value retains `E2002`; whitespace around the named delimiter does not create a named argument; positional-after-named remains early `E2001` | `crates/scribium-markdown/tests/call_grammar_audit.rs::audit_aligns_named_argument_identifier_lexing_and_spans`, `audit_requires_adjacent_named_argument_delimiters_and_preserves_source`, `audit_records_current_early_rejection_of_positional_after_named`; existing malformed argument tests | No semantic conformance claim | `PARTIAL` | #157 implemented; #163 representation ownership; #149 binder/value ownership; #60/#65 | Keep the lexical adjacency contract here; preserve positional/named order through grammar/frontend under #163 and leave semantic validity to #149 | `scribium-quarkdown`; `scribium-markdown` representation; #149 semantic owner |
...
| Source provenance | Accepted call, argument, named-name, segment, body, UTF-8, and CRLF spans are verified; separator-placement truncation, escaped-delimiter truncation, malformed inline recovery, nested tight content, and early mixed-argument rejection remain non-lossless/pinned-equivalent gaps. |
...
- `docs/SYNTAX.md` now records the pinned call-identifier, numeric-reference,
  call-start boundary, and adjacent named-delimiter contract, while retaining
  separate notes for the remaining #158–#164 gaps.
...
- The remaining production gaps are tracked as #158–#160 and #162–#164; they
