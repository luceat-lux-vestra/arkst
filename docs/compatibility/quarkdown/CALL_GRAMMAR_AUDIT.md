...
- after-argument backslash continuation accepts arbitrary leading spaces/tabs
  and LF/CRLF in the revalidated #60/#65 slice; #164 now applies the shared
  separator probe before the first argument, between arguments, and before
  `::`, while its separate root suffix consumes exactly one trailing
  continuation;
...
documentation overclaim. The lexical gaps in [#157], [#158], [#159],
[#160], [#162], and [#164] are now implemented at the grammar/frontend
boundary below. The broader #148 rows remain conservative where their
semantic, content, or protected-context boundaries are still partial:
...
| [#159](https://github.com/luceat-lux-vestra/arkst/issues/159) | Implemented bounded frontend recovery: after recording the existing `E2003`, the malformed extension's original source segment is retained as source-backed `Inline::Text`, including UTF-8 and real CRLF bytes. This is parser/frontend evidence only; it makes no semantic or output compatibility claim. | `arkst-markdown` inline conversion |
...
| [#162](https://github.com/luceat-lux-vestra/arkst/issues/162) | Implemented: escaped call/argument delimiters follow the pinned v2.5.1 `unescapedMatch()` and balanced-brace semantics; escaped braces remain source-backed content and do not alter depth. | `arkst-quarkdown` grammar; `arkst-markdown` integration |
...
| [#164](https://github.com/luceat-lux-vestra/arkst/issues/164) | Implemented bounded separator placement: optional horizontal whitespace and LF/CRLF continuation are transactionally probed before every candidate argument and before `::`; the separate root-level trailing production consumes exactly one valid continuation without fabricating an argument. | `arkst-quarkdown` grammar; `arkst-markdown` block/inline integration |

## #164 implementation reconciliation

Issue [#164](https://github.com/luceat-lux-vestra/arkst/issues/164) is
implemented on branch `compat/164-argument-separators` from the exact baseline
`01c820a56eea92771241cb052a4177c2d6873e26`, against pinned Quarkdown v2.5.1
revision `107ec3a9482f10d6f90d7580f8409b46a719d18e`.

The production change keeps one private `arkst-quarkdown` separator scanner as
the bounded equivalent of the upstream `argumentSeparator`. It consumes
horizontal whitespace, backslash+LF continuation with optional indentation,
and Arkst's existing backslash+CRLF local behavior. The scanner is reused before
the first argument, between arguments, and before each chain `::`; its probe is
committed only when the following argument or chain separator is accepted. A
separate root-level `trailingLineContinuation` consumer accepts exactly one
final continuation and optional indentation without requiring or fabricating an
argument. That suffix widens only the complete call span: head, chain-segment,
and argument spans remain their own source-backed ranges, and a non-argument
remainder stays available to the outer frontend.

Focused parser and frontend tests cover positional and named first arguments,
continuation between arguments, trailing continuation, whitespace and
continuation before `::`, multiple chains, tight-call delegation, and block and
inline placement. They assert complete-call, head, segment, argument, named
name/value, prefix, and suffix byte ranges against the unchanged original
source. Markdown mode remains isolated from all corrected Quarkdown forms.
The pinned continuation evidence is LF-specific; actual CRLF cases are retained
as Arkst source-preservation/frontend evidence and are not described as raw-CRLF
upstream compatibility. This is parser/frontend compatibility evidence only;
binding, evaluator value flow, IR, renderer, and output compatibility remain
separate.
...
boundary row, #162 is the bounded reconciliation of escaped delimiter depth,
and #163 is the bounded reconciliation of its ordered mixed argument
representation; the tests named below remain parser/provenance evidence only.
...
## #162 implementation reconciliation

At the requested base, `arkst-quarkdown::parse_braced` now ignores a `{` or
`}` immediately preceded by `\` when scanning balanced argument depth outside
quoted strings. The scanner otherwise retains its existing quoted-string
behavior and returns the original argument/content byte spans unchanged. The
escaped call introducer remains literal through the existing call-boundary
logic; no call, chain, continuation, binder, evaluator, IR, backend, or
dependency behavior changed.

Independent parser/frontend evidence is in
`arkst-quarkdown/src/lib.rs::escaped_brace_delimiters_remain_source_backed_content`
and
`arkst-markdown/tests/call_grammar_audit.rs::audit_verifies_escaped_delimiter_source_preservation_and_mode_isolation`.
Those tests cover escaped closing/opening/pairs, UTF-8, actual CRLF, exact
source-backed spans, surrounding source, and `.md`/`.qd` isolation. This is
parser/frontend evidence only; it makes no semantic or rendered-output
compatibility claim.
...
| Escaped call/argument delimiters | Public syntax page; pinned `GrammarUtils.kt::unescapedMatch` / `balancedDelimitersMatch` and `FunctionCallGrammar.kt` | `\.foo {x}`; `.foo {a \} b}`; `.foo {a \{ b}`; nested, UTF-8, and CRLF variants | `arkst-quarkdown::parse_call`, `parse_arguments`, and `parse_braced`; escaped `.` remains literal and escaped braces remain content without changing balanced-brace depth | Call, argument, and content spans remain direct byte ranges in the original source; UTF-8/CRLF content is not truncated | Quarkdown probes are covered; Markdown mode keeps all probes literal and isolated | LF, UTF-8, and actual-CRLF source-backed spans are asserted | Genuine unescaped unclosed braces still report `E2003`; escaped delimiters do not produce false closure or `E2003` | `arkst-quarkdown/src/lib.rs::escaped_brace_delimiters_remain_source_backed_content`; `crates/arkst-markdown/tests/call_grammar_audit.rs::audit_verifies_escaped_delimiter_source_preservation_and_mode_isolation` | Independent parser/frontend evidence now exists for the bounded corrected behavior | `PARSED_ONLY` | #162 implemented; #60/#65 boundary evidence | Preserve pinned escaped-delimiter depth semantics and source provenance; #164 separator placement remains separate | `arkst-quarkdown`; `arkst-markdown` integration |
...
| Line continuation and argument-separator placement | Public syntax page; pinned `FunctionCallGrammar.kt` `argumentSeparator` before every inline argument and `trailingLineContinuation`; pinned `FunctionCallTest.kt` continuation-before-first-argument/body and trailing-range evidence | `.foo \\\nname:{x}`; `.foo {x} \\\n` | `parse_arguments` transactionally probes the shared private separator before the first and subsequent arguments; `parse_directive_at` commits it before each `::`; the separate root suffix consumes exactly one final continuation without an argument | Complete calls, arguments, named names/values, and chain ranges point into the unchanged source; the trailing continuation is included only in the complete call range while head, segment, and argument spans are not widened; non-argument remainder is left unconsumed | Positive block/inline Quarkdown probes pass; Markdown mode keeps the same source literal and isolated | The pinned continuation evidence is LF-specific. Actual CRLF cases pass as Arkst source-preservation/frontend evidence and are not raw-CRLF upstream conformance claims | Genuine malformed chain syntax retains source-backed `E2004`; valid continuation suffixes followed by remainder do not; unclosed arguments retain `E2003` | `parses_argument_separators_before_first_argument_and_at_trailing_edge`, `parses_separator_before_chain_without_changing_segment_spans`, `trailing_continuation_accepts_optional_indentation_without_extra_argument`, `trailing_continuation_is_root_suffix_and_leaves_remainder`, `audit_accepts_continuation_before_first_argument_and_preserves_spans`, `audit_accepts_trailing_continuation_without_fabricating_an_argument`, `audit_continuation_suffix_can_start_a_source_backed_block_body`, `audit_accepts_whitespace_and_continuation_before_chain_separators` | Independent parser/frontend evidence for the bounded corrected placement; no evaluator/output claim | `PARSED_ONLY` | #164 implemented; #60/#65 | Keep binding, evaluation, chain value flow, and output compatibility separate | `arkst-quarkdown`; `arkst-markdown` integration |
| Nested calls in arguments | Public syntax page and pinned refiner expression boundary | `.outer {.inner {value}}` | Ordinary and tight nested calls, including calls nested in supported Markdown content, are structurally found in source-backed content; evaluation is not this audit | Outer, inner, and argument spans are source-backed | `.qd` path covered; `.md` remains literal | UTF-8/CRLF surrounding spans covered | Inner malformed diagnostics retain original offsets; malformed inline source recovery is covered by #159 | `parses_nested_content_and_scalar_classification`, `nested_content_calls_keep_prefix_suffix_and_original_spans`, `audit_preserves_nested_tight_call_wrapper_inside_content_argument`, `audit_preserves_nested_tight_utf8_and_crlf_provenance`, `content_inline_markdown.rs` | No end-to-end claim in #148 | `PARSED_ONLY` | #60/#65; #159/#158/#160/#162 | Dynamic/content conversion remains #166; #164 separator placement is implemented separately | `arkst-markdown` |
| `::` chaining grammar | Public syntax page; pinned `FunctionCallGrammar.kt` `chainCallParser` optional `argumentSeparator` before `::`, plus `FunctionCallChainingTest.kt` | `.a {x}::b {y}::c`; `.a {x} ::b {y}`; `.a {x} \\\n::b {y}` | Direct, whitespace-separated, continuation-separated, and multiple `::` segments are structurally retained by the shared separator scan; chain value flow remains outside this grammar audit | Head span remains the first call segment; each chain segment and its arguments retain exact source-backed spans; the complete call includes separators and all segments | `.md` isolation and `.qd` block/inline probes are covered by the audit tests | LF continuation is the pinned upstream evidence; actual CRLF surrounding/continuation cases are local Arkst source-preservation evidence only | Malformed `.a::`, `.a:::b`, and `.a:: {x}` retain deterministic source-backed `E2004` | `parses_separator_before_chain_without_changing_segment_spans`, `rejects_malformed_chains_deterministically`, `audit_accepts_whitespace_and_continuation_before_chain_separators` | Independent parser/frontend evidence for the bounded separator placement; no chain value-flow or output claim | `PARSED_ONLY` | #164 implemented; #60/#65; #61 | Chain semantics/value flow remain separately owned by #150 | `arkst-quarkdown`; `arkst-markdown` integration / #150 |
...
| Inline vs block placement | Public syntax page; pinned `FunctionCallPatterns.kt` block/inline rule | `Text .foo {x} text` vs `.foo {x}` | Block parser accepts isolated calls; inline parser preserves calls inside paragraphs and trailing same-line text selects inline path, including #164 separator forms | Call/paragraph spans remain source-backed and surrounding prefix/suffix are not consumed | `.md` has no directive extension; `.qd` body/list/blockquote paths tested | LF/CRLF covered as source-backed frontend evidence | Malformed inline recovery retains the consumed original segment and following source (#159) | `qd_mode_parses_root_and_inline_calls_with_crlf_provenance`, `audit_accepts_continuation_before_first_argument_and_preserves_spans`, `audit_accepts_trailing_continuation_without_fabricating_an_argument`, `audit_accepts_whitespace_and_continuation_before_chain_separators` | Independent parser/frontend placement evidence only | `PARSED_ONLY` | #60/#65; #159/#162/#164 implemented | Keep placement heuristic independent of evaluator and output | `arkst-markdown` |
...
| Malformed input and recovery | Public grammar requires balanced syntax; Arkst Engineering requires no panic/input loss | `.foo {`, `.foo }`, `.foo {a`, `.foo {a}::` | Structured `E2003`–`E2004` diagnostics are deterministic; malformed inline recovery retains the extension's original source segment as literal source-backed text | Diagnostic spans and recovered AST text are source-backed; the E2003 span remains brace-to-source-end | Code/verbatim shielding remains intact; `.md` stays literal | UTF-8/real-CRLF diagnostic and recovery offsets pass | Genuine unclosed braces remain `E2003`; genuine malformed chains remain `E2004`; valid #164 separator probes and root suffixes do not produce a false diagnostic | `malformed_root_block_reports_argument_span`, `malformed_inline_call_preserves_full_source_offset`, `malformed_inline_recovery_preserves_utf8_crlf_and_following_source`, `valid_inline_call_still_preserves_trailing_text`, `rejects_malformed_chains_deterministically`, `trailing_continuation_is_root_suffix_and_leaves_remainder` | Independent parser/frontend recovery evidence only; no semantic or output claim | `PARSED_ONLY` | #159/#162/#164 implemented | Keep source recovery local to `arkst-markdown`; do not reparse synthetic fragments or change block recovery | `arkst-markdown` |
| Markdown structure inside content arguments | Public Markdown-content page; public call syntax nested-content contract | `.outer {**bold** and [link](target)}` | Supported static Markdown inline nodes are converted through the existing Rushdown frontend lifecycle; ordinary text, emphasis, strong, code, links, images, nested structure, nested calls, UTF-8, and CRLF retain source-backed spans without `E3010` | Content-argument, delimiter-inclusive structured-node, child, link/image, and nested-call spans are checked against the complete original source | `.md` has no Quarkdown directive semantics; `.qd` content conversion uses the same Markdown substrate plus Quarkdown inline extensions | LF, UTF-8, and real CRLF source slices are independently asserted | Malformed Markdown remains Rushdown-owned deterministic text/fallback behavior; unsupported raw HTML and the established opaque angle-text boundary retain the source-text/`E3010` policy; bounded malformed Quarkdown recovery retains its original source segment under #159 | `content_argument_preserves_supported_markdown_structure_and_spans`, `content_inline_markdown.rs` | Independent parser/frontend provenance evidence only; no evaluator or output claim | `PARSED_ONLY` | #160 implemented; related #154 handoff | Dynamic String/content conversion and broader producer/output support remain separate | `arkst-markdown` |
...
| Source provenance | Accepted call, argument, named-name, segment, body, UTF-8, CRLF, escaped-delimiter, nested tight wrapper/inner spans, malformed inline recovery, and #164 separator placement are verified at the frontend boundary. Mixed argument order is retained at the grammar/frontend boundary, and the temporary engine guard reports from the actual argument spans before split IR projection; complete binder validation is not claimed. |
...
- `docs/SYNTAX.md` now records the pinned call-identifier, numeric-reference,
  call-start boundary, adjacent named-delimiter, ordered mixed-argument, and
  #164 separator-placement contract. The #159 recovery, #160 content-inline,
  and #162 escaped-delimiter frontend slices are recorded in the matrix.
...
- #164's bounded separator-placement correction, #159's parser/frontend
  recovery, #160's content-inline slice, and #162's escaped-delimiter
  correction are implemented, while broader content conversion remains
  separately owned. No gap is hidden with expected-failure tests or
  compatibility allowlists in this audit.
