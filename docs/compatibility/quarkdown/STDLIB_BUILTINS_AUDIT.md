...
- Evidence/status: existing scalar and content tests cover whitespace
  ...
  uses direct Temurin 25/Unicode 16 simple upper/lower mappings for Kotlin/JVM-compatible
  ...
  bounded carrier.
- The engine uses a generated engine-local table captured directly from the
  public `Character` and `String` APIs of the pinned Eclipse Temurin
  `25.0.4.1+1` runtime. The generation helper exhaustively captures simple
  scalar mappings, UTF-16 `Char` mappings, full mappings, and Kotlin's
  `Char.titlecase()` algorithm; the transient oracle is never a runtime
  dependency. Full and simple mappings remain distinct, and compile-time plus
  generation-time fingerprints reject mapping-version or provenance drift.
...
continuation, selected conditional/function body replacement with caller continuation, and rc=0. Closed/unmerged PR #386 fixes the strict native host-finalization boundary across the same exact releases: evaluation/log side effects retain ordinary continuation semantics, function/selected-conditional local bodies stop at the error while caller/outer content continues, unselected conditionals stay lazy, the first selected error determines the bounded three-line stderr prefix, exit is 66, and no artifact is published. Arkst models these bounded contracts with a backend-neutral explicit-error component plus default/strict native host policies. Arkst-specific `check`/`inspect` use a separate explicit sinkless/fail-closed analysis policy: no `LogSink`, `.log` is E3010, `.debug` is silent, and `.error` is fatal E3011. `check` preserves all-diagnostics stderr output; `inspect` surfaces fatal diagnostics before refusing output and keeps successful warning-only inspection stderr-silent. Closed/unmerged #392/#393 pin the initial nested structural error-output contexts. Merged #398 supplies bounded independently sourced `.error` source echo; #401 extends exact cross-version evidence to top-level inline, ordered-list, Stacked row/column/grid, Landscape, and a representative `.center` → `.row` composition; #431/#433 establish direct heading/table-cell plus compositional emphasis/strong/strikethrough/link owners. Exact v2.5.1/v2.6.0 source review confirms recursive nestable-owner traversal without a depth counter, so Arkst composes the already-supported rendered-owner tree recursively rather than treating deeper permutations as separate gaps. Distinct semantic boundaries remain fail-closed; image-alt is #182-dependent, and public WASM/embedder exposure remains #191-owned; operation-generated Collection projections are explicitly fail-closed as a non-portable JVM boundary rather than an additional production target; exact Quarkdown HTML/CSS styling is not a Typst/PDF closure requirement. The
...
without a sink; `.error` emits one structured `E3011` diagnostic plus a
backend-neutral explicit-error component. The invocation retains failure-style
...
- #172 closes the bounded Unicode string-semantics gap: `.capitalize` now
  reproduces Kotlin `Char.titlecase()` over full/simple mappings generated
  from the pinned Eclipse Temurin `25.0.4.1+1` oracle (Unicode 16.0), and
