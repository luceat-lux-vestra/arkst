...
| Project data/file identity | `.filename {path}`; `.listfiles {path} ... fullpath:{false} sortby:{none}` | Existing source/asset logical basename identity plus non-empty source-relative directory enumeration with bounded directories/recursive options; no host path/timestamp exposure | Partial (bounded #189 slices; `.csv`/`.bibliography`, regex/name/timestamp/fullpath parity, global-read, and WASM parity remain separate) |
