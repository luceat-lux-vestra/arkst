...
| Localization tables and lookup | `.localization {name}` with a dictionary body; `.localize {table:key}` | Dedicated evaluator-native table state with shared binding, canonical Dictionary/String/Boolean conversion, deterministic #173 locale resolution, atomic merge/replacement, exact first-separator lookup, seeded bounded `std`, and #167 savepoint rollback | Semantically supported (bounded v2.5.1 slice; no renderer/output-equivalence or exhaustive resource-data claim) |
...
localization mutation and exact lookup consume this evaluator state without
changing `.doclang` ownership; they are not #152 semantics.
