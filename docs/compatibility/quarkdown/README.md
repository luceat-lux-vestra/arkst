...
generic String conversion. Native CLI build `.log` stdout integration is now
wired through the explicit sink boundary. #197 stays open for rendered-error/
stderr/strict behavior and unreviewed logger String categories; public WASM/
embedder exposure remains deferred under #191, and #190 is complete. For #154, `.match` is #198-owned,
...
| Project data/file identity | `.filename {path}`; `.listfiles {path} ... fullpath:{false} sortby:{none|name}` | Existing source/asset/logical-directory basename identity plus inferred, explicit, and empty source-relative logical directory enumeration; `sortby:none` keeps set-like bare-name semantics and ASCII `sortby:name` provides pinned lowercase/alphanumeric ascending/descending ordering with duplicate preservation; no host path/timestamp exposure | Partial (bounded #189 slices; `.csv`/`.bibliography`, regex/Unicode-name/timestamp/fullpath parity, global-read, and WASM parity remain separate) |
...
The remaining unimplemented v2.5.1 data-file families (`.csv`, `.bibliography`) and `.llmstxt` retain explicit gap/defer ownership above. Bounded `.includeall`,
`.filename`, and `.listfiles` implementations are `PARTIAL` under the canonical
#155 manifest; those bounded slices must not be read as full compatibility.
