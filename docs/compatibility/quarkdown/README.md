...
| `.json` data loading           | `.json {path}` (new in v2.5.0)   | UTF-8 JSON mapped to recursive typed `IrValue` collections/dictionaries/scalars; exact binary64 integer boundary; logical resource diagnostics | Partial (bounded source-relative resource slice; #155; resolver prerequisite completed under #188; native host ingestion completed under #298/#302; residual owners #296/#191) |
