...
#403/#405: origin determines whether the logger sees direct-list Markdown text, and the direct
projection preserves source spelling after the existing `trimIndent().trimEnd()` body boundary
(marker choice/spacing, ordered numbering, strong/emphasis source, one nested unordered-list
level, task markers, and multiline continuation). Arkst retains that raw body only as
evaluator-local metadata and exposes it only to a same-scope direct variable reference; aliases,
callable captures/results, directive-bearing bodies, and operation-generated Collections remain
fail-closed, as does generic `IrValue::Collection` stringification. Native `arkst build` writes `Log` events to stdout through an explicit
...
audit is documentation/guard-only. The #155 → #156 → #187 sequence is
historical provenance; #187, #200, and #201 are completed and are not current
follow-up owners.
...
| Malformed-call diagnostics     | `E2003`, `E2004` | Error                  | Partial: incomplete optional named candidates and valid #164 trailing suffixes remain source remainder; bounded inline recovery retains the original source segment after `E2003`, and genuine malformed chains retain `E2004` |
...
| Dictionary lookup              | `.get {dictionary} {key} orelse:{fallback}` and `dictionary::get {key}` | Dedicated evaluator-native lookup with shared `dictionary`/`key`/`orelse` binding, canonical String-key conversion, exact ordered equality, typed stored/fallback values, and `None` for an omitted fallback | Semantically supported (bounded v2.5.1 slice; no renderer/output-equivalence claim) |
| Localization tables and lookup | `.localization {name}` with a dictionary body; `.localize {table:key}` | Dedicated evaluator-native table state with shared binding, canonical Dictionary/String/Boolean conversion, deterministic #173 locale resolution, atomic merge/replacement, exact first-separator splitting with exact table/locale lookup and pinned whole-string Kotlin/JVM lowercase-first-then-original key lookup (`entries[key.lowercase()] ?: entries[key]`, including contextual Final_Sigma and not general case-insensitive matching), seeded bounded `std`, and #167 savepoint rollback | Semantically supported (bounded v2.5.1 slice; no renderer/output-equivalence or exhaustive resource-data claim) |
...
| Include/read                   | `.include {path}`, `.read {path}` with optional `lines` range | Source-relative logical `VirtualProject` resources; included sources retain their own source identity and working directory; active-stack cycle detection; no host filesystem or network access | Partial (bounded subset; resolver prerequisite completed under #188) |
| Project data/file identity | `.filename {path}`; `.listfiles {path} ... fullpath:{false} sortby:{none|name}`; bounded raw `.csv` resource parsing | Existing source/asset/logical-directory basename identity plus inferred, explicit, and empty source-relative logical directory enumeration; `sortby:none` keeps set-like bare-name semantics and ASCII `sortby:name` provides pinned lowercase/alphanumeric ascending/descending ordering with duplicate preservation; CSV resources can be source-relatively loaded and parsed into bounded trimmed raw columns without yet producing a public table node; no host path/timestamp exposure | Partial (bounded #189 slices; public `.csv` callable/table semantics are #183-owned, `.bibliography` and regex/Unicode-name/timestamp/fullpath parity remain #189 residuals, and global-read/WASM parity remain separate) |
| Metadata                       | `.doctype`, `.docname`, `.docdescription`, `.docauthor`, `.docauthors`, `.dockeywords`, `.doclang`, `.theme`, and related document metadata | Canonical Issue #152 classification is eight `PARTIAL` evaluator/IR slices (`.doctype`, `.docname`, `.docdescription`, `.docauthor`, `.docauthors`, `.dockeywords`, `.doclang`, `.theme`). `.localization`/`.localize` remain #151-owned semantics and are implemented separately by #196; renderer, front-matter, and cross-owned layout/content/resource state remain separate | [#152 audit](DOCUMENT_STATE_AUDIT.md), [#151 manifest](STDLIB_BUILTINS_AUDIT_MANIFEST.tsv), and [#196 evidence](STDLIB_BUILTINS_AUDIT.md) |
| Layout/document configuration | `.numbering`, `.nonumbering`, `.font`, `.paragraphstyle`, `.captionposition`, `.texmacro`, `.pageformat`, `.pagemargin`, `.footer`, page counters/format/reset, `.lastheading`, automatic page breaks, `.marker`, `.navigation`, `.tableofcontents`, `.slides` | Canonical #153 result: 9 conservative `PARTIAL` bounded slices (`.captionposition`, `.numbering`/`.nonumbering`, bounded size-only `.font`, `.paragraphstyle`, bounded `.pageformat`, automatic page-break setters, bounded `.slides`) and 11 `PARSED_ONLY` rows; no complete #153 output-equivalence claim | [#153 audit](LAYOUT_DOCUMENT_CONFIGURATION_AUDIT.md) |
...
| Line continuation (`\`)        | `\` at end of line               | Parsed                   | Parsed-only bounded parser/frontend support: separator probes before the first/subsequent arguments and `::` commit only on an accepted candidate, while a separate root suffix consumes exactly one trailing continuation; LF is pinned evidence, actual CRLF is separate local source-preservation evidence |
...
| `.json` data loading           | `.json {path}` (new in v2.5.0)   | UTF-8 JSON mapped to recursive typed `IrValue` collections/dictionaries/scalars; exact binary64 integer boundary; logical resource diagnostics | Partial (bounded source-relative resource slice; #155; resolver prerequisite completed under #188; native host ingestion completed under #298/#302; accepted global-read policy divergence (completed #296/#300) plus residual owner #191) |
...
is unchanged. Arkst does not let `.doclang` own or mutate localization tables,
and introduces no hyphenation, Typst/HTML language output, or locale-aware
...
initial localization table contains the seeded `std` table. The #196-owned
localization mutation and lookup consume this evaluator state without
changing `.doclang` ownership; they are not #152 semantics. Table names and
locale tags are matched exactly, and `.localize` key lookup follows Quarkdown
v2.5.1's `BaseContext.localize()` contract of `entries[key.lowercase()] ?:
entries[key]` -- the pinned whole-string Kotlin/JVM-compatible lowercase form
of the key, including contextual Final_Sigma behavior, is tried first,
falling back to the original key only if that lookup misses.
This is not general case-insensitive lookup.
...
20-case independent corpus is intentionally bounded and does not imply that
...
| Dictionary lookup | `arkst-engine/src/evaluator.rs::dictionary_lookup_*`; `arkst-core/tests/stdlib_builtin_audit.rs::dictionary_get_semantics_are_observable_through_the_public_facade`; `fixtures/quarkdown-conformance/cases/dictionary-get-family/` | `.get` preserves typed stored values and typed `orelse` fallback values, applies exact String-key lookup over ordered `IrDictionary.entries`, supports direct/chain/variable-held calls, and fails closed with source-backed diagnostics; no output-equivalence claim | `SUPPORTED_SEMANTICS` |
| Localization tables and lookup | `arkst-core/tests/quarkdown_localization.rs`; `arkst-test-support/src/lib.rs::tests::quarkdown_conformance_corpus_obeys_declared_levels`; `fixtures/quarkdown-conformance/cases/localization-family/`; independently authored black-box probes in [`LOCALIZATION_PROBES.md`](LOCALIZATION_PROBES.md) | `.localization` validates typed locale/dictionary/string values before atomic evaluator-state publication, supports merge precedence and seeded `std`; `.localize` consumes `.doclang`, splits only at the first separator boundary, uses exact table/locale lookup and lowercase-first-then-original key lookup (not general case-insensitive matching), and fails closed without locale fallback; no output-equivalence or exhaustive seed-data claim | `SUPPORTED_SEMANTICS` |
...
- Pinned v2.5.1 permits optional argument separators before the first argument
  ...
  Arkst now transactionally probes those forms in the bounded
  `arkst-quarkdown` grammar and uses a separate root-level suffix for exactly
  one trailing continuation, leaving a non-argument remainder available to the
  outer `arkst-markdown` lifecycle. The pinned
...
The remaining unimplemented v2.5.1 bibliography family and `.llmstxt` retain explicit gap/defer ownership above. `.csv` now has only a bounded raw resource/parser slice; its public callable/table producer remains #183-owned. Bounded `.includeall`, `.filename`, `.listfiles`, and `.csv` rows are `PARTIAL` under the canonical #155 manifest; those bounded slices must not be read as full compatibility.
