...
| Call chaining (`::`)           | `.a {x}::b {y}`                  | Semantically supported for the evidenced `.sum`, `.multiply`, `.uppercase`, and `.lowercase` chain callees; strict left-to-right value flow with source-backed `E3001` failures for unimplemented callees | Implemented (evidenced slice) |
...
callees. The successful block, inline, content-argument, provenance, failure,
and Typst/PDF tests listed above support this slice only; general programmable
document compatibility is not claimed here.
...
evidenced `.sum`, `.multiply`, `.uppercase`, and `.lowercase` chain forms are
**Semantically supported** with strict left-to-right value flow; an
unimplemented chain callee reports a source-backed `E3001` evaluation error.
