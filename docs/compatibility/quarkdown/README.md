...
| Localization tables and lookup | `.localization {name}` with a dictionary body; `.localize {table:key}` | Dedicated evaluator-native table state with shared binding, canonical Dictionary/String/Boolean conversion, deterministic #173 locale resolution, atomic merge/replacement, exact first-separator splitting with exact table/locale lookup and lowercase-first-then-original key lookup (`entries[key.lowercase()] ?: entries[key]`, not general case-insensitive matching), seeded bounded `std`, and #167 savepoint rollback | Semantically supported (bounded v2.5.1 slice; no renderer/output-equivalence or exhaustive resource-data claim) |
...
entries[key]` -- the pinned Kotlin/JVM-compatible lowercase form of the key is
tried first, falling back to the original key only if that lookup misses.
