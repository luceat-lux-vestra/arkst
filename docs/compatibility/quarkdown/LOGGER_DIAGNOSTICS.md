...
observed Markdown-list Collection conversion, but Arkst does not implement that
case in this slice because `IrValue::Collection` no longer retains enough
ordered-vs-unordered/source-list origin to keep the production adapter bounded.
#390 is closed unmerged and MUST NOT MERGE.
...
- close #197 while still-unevidenced output contexts and remaining logger String categories such as Collection remain;
