...
an unordered Markdown-list variable projects as Markdown list text, while
`reversed`, `distinct`, `sorted`, Range/Pair adaptation, `foreach`, and
`groupvalues` produce different JVM object-style projections, some containing
unstable runtime context identities. Arkst therefore supports only the exact
evaluator-retained direct Markdown-list origin and keeps operation-generated
`IrValue::Collection` values fail-closed. #390 and #403 are closed unmerged
and MUST NOT MERGE.
