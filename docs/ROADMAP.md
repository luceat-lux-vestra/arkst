...
model has `SUPPORTED_SEMANTICS` evidence, while `.read`, `.json`, `.include`,
`.filename`, and bounded `.listfiles` remain `PARTIAL` source-relative builtin rows owned by #155. The common
