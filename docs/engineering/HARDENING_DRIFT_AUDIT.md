...
`tools/ci/test_hardening_drift_audit.py` contains safe negative fixtures for classification, repository settings, release-tag protection, CodeQL authority, supply-chain reporting, security/ownership documentation, and reporter create/update/reopen/recovery/duplicate behavior. The existing required `fmt` job executes these tests on every pull request.
